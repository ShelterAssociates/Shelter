...
class SponsorProjectDetailsAdmin(admin.ModelAdmin):
    ...
    def check_for_duplicates(sender, instance, **kwargs):
        ...
        if len(unique_hh_codes) != len(household_codes):
            message_dict["Message"] = (
                "There are duplicate households present in your data !!"
            )
        else:
            ...
            if (
                instance.id
            ):  # We are calculating this to find out change household numbers.
                _, sp_change = SponsorProjectDetailsSubFields.objects.filter(
                    id=instance.id
                ).values_list("sponsor_project_details", "household_code")[0]
                hh_change = set(change_data).difference(set(sp_change))

            sp_project_details_code = (
                SponsorProjectDetails.objects.filter(slum_id__name=instance)
                .exclude(sponsor_id=10)
                .values_list("id", flat=True)
            )
            sub_fields = SponsorProjectDetailsSubFields.objects.filter(
                sponsor_project_details__in=sp_project_details_code
            )
            ...
            for i in sub_fields:
                if instance.id is None:
                    ...
                    if len(diff) != len(change_data):
                        ...
                        message_dict["Message"] = (
                            "These household no's. already available in Sponsor data. Kindly check !!{}".format(
                                common_hhs
                            )
                        )
                else:
            ...
            if len(hh_codes) > 0:
                ...
                Toilet_data = ToiletConstruction.objects.filter(
                    slum_id__name=instance, household_number__in=change_data
                ...
                if Toilet_data.count() > 0:
                    if Toilet_data.count() != len(change_data):
                        ...
                        message_dict["Message"] = (
                            "The Sanitation data is not available for these households. Kindly check !! {}".format(
                                hh_codes
                            )
                        )
                else:
    ...
    def response_add(self, request, obj, post_url_continue=None):
        if message_dict:
            ...
            return HttpResponseRedirect(
                reverse("admin:sponsor_sponsorprojectdetails_changelist")
            )
        else:
    ...
    def response_change(self, request, obj):
        if message_dict:
            ...
            return HttpResponseRedirect(
                reverse("admin:sponsor_sponsorprojectdetails_change", args=[obj.pk])
            )
        else:
...
class SponsorProjectAdmin(admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        type_options = filter(
            lambda x: search_term.lower() in x[1].lower(), TYPE_CHOICES
        )
        type_options = map(lambda x: x[0], type_options)
        status_options = filter(
            lambda x: search_term.lower() in x[1].lower(), PROJECTSTATUS_CHOICES
        )
        status_options = map(lambda x: x[0], status_options)
...
class SponsorProjectMOUAdmin(admin.ModelAdmin):
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        :return: queryset
         ...
         Function for adding custom filters for choice field options
        """
        queryset, use_distinct = super(SponsorProjectMOUAdmin, self).get_search_results(
            request, queryset, search_term
        )

        quarter_options = filter(
            lambda x: search_term.lower() in x[1].lower(), QUARTER_CHOICES
        )
        quarter_options = map(lambda x: x[0], quarter_options)
        queryset |= self.model.objects.filter(quarter__in=quarter_options)
        return queryset, use_distinct
...
class UserAdminCust(UserAdmin):
    ...
    def response_change(self, request, obj):
        if obj.groups.filter(name="sponsor").exists():
            ...
            return HttpResponseRedirect(
                "../../../sponsor/sponsor/" + str(sponsor.id) + "/"
            )
        else:
