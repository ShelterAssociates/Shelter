...
class Sponsor(models.Model):
    ...
    user = models.ForeignKey(
        User, limit_choices_to={"groups__name": "sponsor"}, on_delete=models.DO_NOTHING
    )
...
TYPE_CHOICES = (("1", "Intervention"), ("2", "Collection"))

PROJECTSTATUS_CHOICES = (("1", "Planned"), ("2", "Activated"), ("3", "Closed"))
...
class SponsorProject(models.Model):
    ...
    funds_utilised = models.DecimalField(
    ...
    sponsor = models.ForeignKey(
        Sponsor, null=True, blank=True, on_delete=models.CASCADE
    )
...
SPONSORSTATUS_CHOICES = (("1", "Initiated"), ("2", "In-progress"), ("3", "Completed"))


def zip_path(instance, filename):
    return os.path.join(
        ZIP_PATH, instance.sponsor_project.sponsor.organization_name, filename
    )
...
class SponsorProjectDetailsSubFields(models.Model):
    sponsor_project_details = models.ForeignKey(
        SponsorProjectDetails, on_delete=models.CASCADE
    )
    household_code = JSONField(null=True, blank=True)
    ...
    def save(self):
        ...
        if self.status == SPONSORSTATUS_CHOICES[2][0]:
            ...
            if (
                self.pk
                and self.status
                == SponsorProjectDetailsSubFields.objects.get(pk=self.pk).status
            ):
                flag = False
...
class SponsorProjectMOU(models.Model):
    ...
    def __str__(self):
        return (
            self.sponsor.organization_name
            + " "
