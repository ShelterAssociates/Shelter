...
def sbmUpdate():
    ...
    sponsored_households = SponsorProjectDetails.objects.filter(
        sponsor__organization_name="SBM Toilets"
    )
    for sponsored_household in sponsored_households:
        ...
        url = (
            ...
            + "data/"
            + kobo_survey
            + '?format=json&query={"group_ce0hf58/slum_name":"'
            + slum_code
            + '"}'
        )
        ...
        grouped_records = itertools.groupby(
            sorted(json_records, key=lambda x: int(x["group_ce0hf58/house_no"])),
        ...
        folder_path = os.path.join(
            output_folder_path, "slum_" + str(slum.replace("/", ""))
        )
        rec = []
        for house in sponsored_household.household_code:
            # house = '000'.join(str(house))[-4:]
            if house in records:
                record_sorted = sorted(
                    records[house], key=lambda x: x["end"], reverse=True
                )
...
def pcmc_rhs_xml_replace(rhs_record):
    ...
    if rhs_record["group_ce0hf58"]["Type_of_structure_occupancy"] == "01":
        rhs_record["group_ye18c77"]["group_yw8pj39"][
