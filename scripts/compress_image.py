...
Input parameters
        -h : help
        -f : folder path
        -s : (optional) files above this size will get compressed (in KBs)
...
fp_success = open("/srv/log/compress_success.log", "w")
fp_error = open("/srv/log/compress_error.log", "w")
...
def getFiles(xml_root_path, above_size, below_size, quality, compress_flag):
        ...
        for filename in files:
            ...
            if m:
                try:
                    ...
                    original_size = os.stat(filepath).st_size / 1024
                    flag = (
                        original_size > above_size
                        if below_size <= 0
                        else original_size > above_size and original_size <= below_size
                    )
                    if flag:
                        if compress_flag:
    ...
    print(
        "Total number of files greater that " + str(above_size) + "KB - " + str(count)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=500,
        help="compress files above size(in KBs)",
    )
    parser.add_argument(
        "-sb",
        "--size_below",
        type=int,
        default=0,
        help="compress files below size(in KBs)",
    )
    parser.add_argument(
        "-q",
        "--quality",
        type=int,
        default=65,
        help="Quality to which the image needs to be compressed",
    )
    parser.add_argument(
        "-c",
        "--compress",
        action="store_true",
        help="Compress the files. If not supplied it will just print the count",
    )
