...
def queryToFetchRecords(kobo_survey):
    ...
    url = (
        ...
        + "data/"
        + kobo_survey
        + '?format=json&query={ "_id":{"$lte":123957}}'
    )
    ...
    url = (
        ...
        + "data/"
        + kobo_survey
        + '?format=json&query={ "_id":{"$gt":123957}}'
    )
    kobotoolbox_request = urllib2.Request(url)
    kobotoolbox_request.add_header("User-agent", "Mozilla 5.10")
    kobotoolbox_request.add_header("Authorization", settings.KOBOCAT_TOKEN)
...
def kmc_rhs_xml_replace(rhs_record, record):
    ...
    if (
    ...
    ):  # and 'group_ye18c77/group_ud4em45/what_is_the_full_name_of_the_family_head_' in record :
        ...
        if re.search("[0-9]+", parent_house) != None:
            ...
            if (
                rhs_record["slum_name"] in DOUBLE_HOUSE.keys()
                and str(rhs_record["Household_number"])
                in DOUBLE_HOUSE[rhs_record["slum_name"]].keys()
            ):
                i = i + 1
                print(str(i))
                rhs_record["Parent_household_number"] = int(
                    DOUBLE_HOUSE[rhs_record["slum_name"]][
                        str(rhs_record["Household_number"])
                    ]
                )
    ...
    if rhs_record["Type_of_structure_occupancy"] == "01":
        ...
        if rhs_record["group_oi8ts04"]["How_many_installments_have_you"] and (
            not rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"]
            or rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "02"
        ):
        ...
        if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "01" and (
            not rhs_record["group_oi8ts04"]["How_many_installments_have_you"]
            or rhs_record["group_oi8ts04"]["How_many_installments_have_you"] == ""
        ):
            # print "Hello"
            rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] = "02"
        ...
        if (
            not rhs_record["group_oi8ts04"]["When_did_you_receive_r_second_installment"]
            and rhs_record["group_oi8ts04"]["When_did_you_receive_ur_first_installment"]
        ):
            if not rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"]:
                rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] = "01"
        ...
        if (
            not rhs_record["group_oi8ts04"]["When_did_you_receive_r_second_installment"]
            and not rhs_record["group_oi8ts04"][
            ...
            if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "02":
                if (
                    not rhs_record["group_oi8ts04"]["How_many_installments_have_you"]
                    and not rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"]
                    or rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] == "05"
                ):
                    c2 = None
            ...
            if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "01":
                if (
                    rhs_record["group_oi8ts04"]["How_many_installments_have_you"]
                    == "02"
                ):
        ...
        if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "01":
            if rhs_record["group_oi8ts04"]["How_many_installments_have_you"] in [
                "02",
                "03",
            ]:
                # print "Pre"
