...
# kobo_survey = "53" # Thane_RHS  #vyfJBzaGMbWskxQjeS2PsH #71757a721d7d4c8680443ca9229d5d00 #aQr8e2GKbZrGUtgpYeLgWv #106
kobo_survey = "65"  # NMMC_RHS  #vi34nTceu5iSvtwFt39MTu  #f5a9a8f583524845aef4c211e9e2ceef  #aqyX397R6ouhYP2CJXJ9FJ
# 131 #vcD228o4tkk4LLjZe4WLVs  #2237f56bf16948599797f4fb7e629adf  #RHS_SBM_NMMC_v1
root_folder_path = os.path.dirname(os.path.abspath(__file__))
root_output_folder = os.path.join(root_folder_path, "xml_output")
output_folder_path = os.path.join(root_output_folder, "NMMC", "RHS")
...
def queryToFetchRecords(kobo_survey):
    ...
    for record in json_records:
        if (
            "group_ye18c77/group_yw8pj39/what_is_the_structure_of_the_house" in record
            and record["group_ye18c77/group_yw8pj39/what_is_the_structure_of_the_house"]
            != ""
        ):
    ...
    grouped_records = itertools.groupby(
        sorted(json_records, key=lambda x: x["group_ce0hf58/slum_name"]),
        key=lambda x: x["group_ce0hf58/slum_name"],
    )
    records = {}
    for list_record in grouped_records:
        slum_house = list(list_record[1])
        grouped_house_records = itertools.groupby(
            sorted(slum_house, key=lambda x: int(x["group_ce0hf58/house_no"])),
    ...
    for k, v in records.iteritems():
        ...
        folder_path = os.path.join(
            output_folder_path, "slum_" + str(slum_code.replace("/", ""))
        )
        for key, val in v.iteritems():
            ...
            for record_sort in record_sorted:
                if "group_ce0hf58/Type_of_structure_occupancy" in record_sort:
                    if record_sort[
                        "group_ce0hf58/Type_of_structure_occupancy"
                    ] == "01" or (
                        record_sort["group_ce0hf58/Type_of_structure_occupancy"] != "01"
                        and flag
                    ):
                        flag = False
                        record = record_sort
                        ...
                        if status:
                            xml_string = create_xml_string(
                                replace_data,
                                [],
                                replace_data["_xform_id_string"],
                                replace_data["_xform_id_string"],
                                replace_data["__version__"],
                            )
                            file_name = (
                                "RHS_Survey_Slum_Id_"
                                + str(slum_code.replace("/", ""))
                                + "_House_code_"
                                + str(key)
                                + "_"
                                + str(record["_id"])
    ...
    for key, val in xml_record.iteritems():
        ...
        if not key.startswith("_"):
            ...
            if split_ky[len(split_ky) - 1] in mapping.keys():
                ...
                if len(split_key) == 4:
                    gl_rhs_xml_dict[split_key[0]][split_key[1]][split_key[2]][
                        split_key[3]
                    ] = val
...
def kmc_rhs_xml_replace(rhs_record, record):
    ...
    # RHS section
    if record["group_ce0hf58/Type_of_structure_occupancy"] in [
        "03",
        "04",
        "05",
        "06",
        ...
        rhs_record["Type_of_unoccupied_house"] = UNOCCUPIED_HOUSE[
            record["group_ce0hf58/Type_of_structure_occupancy"]
        ]
    else:
        rhs_record["Type_of_structure_occupancy"] = (
            "03"
            if record["group_ce0hf58/Type_of_structure_occupancy"] == "02"
            else "01"
        )

    if (
        rhs_record["Type_of_unoccupied_house"]
        and rhs_record["Type_of_unoccupied_house"] == "021"
        and "group_ye18c77/group_ud4em45/what_is_the_full_name_of_the_family_head_"
        in record
    ):

        parent_house = record[
            "group_ye18c77/group_ud4em45/what_is_the_full_name_of_the_family_head_"
        ].strip()  # .split(' ')
        # print record['group_ye18c77/group_ud4em45/what_is_the_full_name_of_the_family_head_']
        if re.search("[0-9]+", parent_house) != None:
            rhs_record["Parent_household_number"] = int(
                re.search("[0-9]+", parent_house).group(0)
            )
    ...
    if rhs_record["group_el9cl08"]["Aadhar_number"] and not re.search(
        "^[0-9]{12}$", rhs_record["group_el9cl08"]["Aadhar_number"]
    ):
        rhs_record["group_el9cl08"]["Aadhar_number"] = ""
    ...
    if rhs_record["Type_of_structure_occupancy"] == "01":
        ...
        if (
            rhs_record["group_el9cl08"]["Number_of_household_members"]
            and int(rhs_record["group_el9cl08"]["Number_of_household_members"]) > 20
        ):
        ...
        if (
            rhs_record["group_el9cl08"]["Number_of_household_members"]
            and int(rhs_record["group_el9cl08"]["Number_of_household_members"]) == 0
        ):
            rhs_record["group_el9cl08"]["Number_of_household_members"] = 5

        if (
            rhs_record["group_el9cl08"]["Do_you_have_any_girl_child_chi"] == "01"
            and int(rhs_record["group_el9cl08"]["How_many"]) == 0
        ):
        ...
        if (
            rhs_record["group_el9cl08"]["Do_you_have_any_girl_child_chi"] == "01"
            and int(rhs_record["group_el9cl08"]["How_many"]) >= 10
        ):
            rhs_record["group_el9cl08"]["How_many"] = 3

        if (
            rhs_record["group_el9cl08"]["Do_you_have_any_girl_child_chi"] == "02"
            and rhs_record["group_el9cl08"]["How_many"]
            and int(rhs_record["group_el9cl08"]["How_many"]) > 0
        ):
            rhs_record["group_el9cl08"]["Do_you_have_any_girl_child_chi"] = "01"
        ...
        if (
            "group_ye18c77/group_yw8pj39/facility_of_waste_collection" in record
            and len(
                record[
                    "group_ye18c77/group_yw8pj39/facility_of_waste_collection"
                ].split(" ")
            )
            > 0
        ):
            ...
            rhs_record["group_el9cl08"]["Facility_of_solid_waste_collection"] = (
                FACILITY_OF_SOLID_WASTE[
                    record[
                        "group_ye18c77/group_yw8pj39/facility_of_waste_collection"
                    ].split(" ")[0]
        ...
        # if rhs_record['group_oi8ts04']['Have_you_applied_for_individua'] == "01" and (
        #     not rhs_record['group_oi8ts04']['How_many_installments_have_you'] or rhs_record['group_oi8ts04'][
        #     'How_many_installments_have_you'] == ""):
        #     print "Hello"
        #     rhs_record['group_oi8ts04']['Have_you_applied_for_individua'] = '02'
        ...
        if (
            "group_ye18c77/group_yw8pj39/what_is_the_status_of_toilet_under_sbm_"
            in record
        ):
            rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] = (
                TOILET_UNDER_SBM[
                    record[
                        "group_ye18c77/group_yw8pj39/what_is_the_status_of_toilet_under_sbm_"
                    ]
        ...
        if "group_ye18c77/group_yw8pj39/type_of_toilet_preference" in record:
            rhs_record["group_oi8ts04"]["What_kind_of_toilet_would_you_like"] = (
                TOILET_PREFERENCE[
                    record["group_ye18c77/group_yw8pj39/type_of_toilet_preference"]
                ]
            )
        ...
        if (
            "group_ye18c77/group_yw8pj39/where_the_individual_toilet_is_connected_to_"
            in record
        ):
            rhs_record["group_oi8ts04"]["What_is_the_toilet_connected_to"] = (
                TOILET_CONNECTED_TO[
                    record[
                        "group_ye18c77/group_yw8pj39/where_the_individual_toilet_is_connected_to_"
                    ]
        ...
        if (
            "group_ye18c77/group_yw8pj39/does_any_member_of_your_family_go_for_open_defecation_"
            in record
        ):
        ...
        if not rhs_record["group_el9cl08"]["Does_any_household_m_n_skills_given_below"]:
            rhs_record["group_el9cl08"][
                "Does_any_household_m_n_skills_given_below"
            ] = "05"
        ...
        if "group_ye18c77/group_yw8pj39/Current_place_of_defecation_toilet" in record:
            ...
            if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "02":
                ...
                c2 = DEFECATION[
                    record[
                        "group_ye18c77/group_yw8pj39/Current_place_of_defecation_toilet"
                    ].split()[0]
                ]
                if c2 != "05":
                ...
                if rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] == "01":
                    ...
                    c1 = "01"
                elif rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] in [
                    "02",
                    "03",
                    "04",
                ]:
        ...
        if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "02":
            ...
            if rhs_record["group_oi8ts04"]["C2"] == "08":
                if (
                    ...
                    rhs_record["group_oi8ts04"][
                        "What_is_the_toilet_connected_to"
                    ] = None
        ...
        if rhs_record["group_oi8ts04"]["Have_you_applied_for_individua"] == "01":
            if rhs_record["group_oi8ts04"]["How_many_installments_have_you"] == "01":
                rhs_record["group_oi8ts04"][
                    "When_did_you_receive_r_second_installment"
                ] = None
                ...
                if rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] == "01":
                    ...
                    rhs_record["group_oi8ts04"]["C1"] = None
                    rhs_record["group_oi8ts04"]["C2"] = "05"
                    if rhs_record["group_oi8ts04"]["C2"] == "05":
                        rhs_record["group_oi8ts04"][
                            "Are_you_interested_in_an_indiv"
                        ] = None
                        rhs_record["group_oi8ts04"]["If_no_why"] = None
                        rhs_record["group_oi8ts04"]["If_yes_why"] = None
                        if not rhs_record["group_oi8ts04"][
                            "What_is_the_toilet_connected_to"
                        ]:
                            rhs_record["group_oi8ts04"][
                                "What_is_the_toilet_connected_to"
                        ...
                        rhs_record["group_oi8ts04"][
                            "What_kind_of_toilet_would_you_like"
                        ] = None
                # 6
                if not rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"]:
                    if (
                        rhs_record["group_oi8ts04"]["Current_place_of_defecation"]
                        == "05"
                    ):
                        rhs_record["group_oi8ts04"][
                            "Have_you_applied_for_individua"
                        ] = "03"
                        rhs_record["group_oi8ts04"]["Type_of_SBM_toilets"] = None
                        rhs_record["group_oi8ts04"][
                            "How_many_installments_have_you"
                        ] = None
                        rhs_record["group_oi8ts04"]["C2"] = rhs_record["group_oi8ts04"][
                            "Current_place_of_defecation"
                        ]

                    if rhs_record["group_oi8ts04"]["Current_place_of_defecation"] in [
                        "09",
                        "10",
                    ]:
                        rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] = "05"
                        rhs_record["group_oi8ts04"]["C4"] = None
                        rhs_record["group_oi8ts04"]["C5"] = rhs_record["group_oi8ts04"][
                            "Current_place_of_defecation"
                        ]

                if rhs_record["group_oi8ts04"]["Status_of_toilet_under_SBM"] in [
                    "03",
                    "04",
                    "05",
                ]:
            ...
            if rhs_record["group_oi8ts04"]["How_many_installments_have_you"] == "02":
                if not rhs_record["group_oi8ts04"][
                    "When_did_you_receive_ur_first_installment"
                ]:
                    if (
                        rhs_record["group_oi8ts04"]["Current_place_of_defecation"]
                        == "05"
                ...
                if rhs_record["group_oi8ts04"][
                    ...
                    if (
                        ...
                        rhs_record["group_oi8ts04"]["C4"] = rhs_record["group_oi8ts04"][
                            "Current_place_of_defecation"
                        ]
                        rhs_record["group_oi8ts04"]["C5"] = None
                        rhs_record["group_oi8ts04"][
                            "Are_you_interested_in_an_indiv"
                        ] = None
                        rhs_record["group_oi8ts04"]["If_no_why"] = None
        ...
        if not rhs_record["group_oi8ts04"]["OD1"]:
            if rhs_record["group_oi8ts04"]["C3"] == "12":
        ...
        #             rhs_record['group_oi8ts04']['Current_place_of_defecation'] = "05"
        #
        #         if not rhs_record['group_oi8ts04']['C5']:
        #             rhs_record['group_oi8ts04']['C2'] = None
        #             rhs_record['group_oi8ts04']['C5'] = rhs_record['group_oi8ts04']['C3']
        ...
        #         rhs_record['group_oi8ts04']['If_no_why'] = None
        #
        #     if rhs_record['group_oi8ts04']['Status_of_toilet_under_SBM'] in ["02","03","04"]:
        #         print "Inside 02,03,04"
        #         rhs_record['group_oi8ts04']['What_kind_of_toilet_would_you_like'] = None
        ...
        #             rhs_record['group_oi8ts04']['Are_you_interested_in_an_indiv'] = "01"
        #             rhs_record['group_oi8ts04']['If_yes_why'] = "01"
        #
        # if rhs_record['group_oi8ts04']['Are_you_interested_in_an_indiv'] == "01" and not rhs_record['group_oi8ts04']['What_kind_of_toilet_would_you_like'] :
        ...
        #             rhs_record['group_oi8ts04']['What_kind_of_toilet_would_you_like'] = "02"
        #             rhs_record['group_oi8ts04']['If_yes_why'] = "01"
        #             rhs_record['group_oi8ts04']['C5'] = rhs_record['group_oi8ts04']['C4']
        #             rhs_record['group_oi8ts04']['C4'] = None
        #     if rhs_record['group_oi8ts04']['Status_of_toilet_under_SBM'] == "05":
        #         if rhs_record['group_oi8ts04']['How_many_installments_have_you'] == "02":
        #             rhs_record['group_oi8ts04']['C4'] = rhs_record['group_oi8ts04']['C5']
        #             rhs_record['group_oi8ts04']['C5'] = None
        #             rhs_record['group_oi8ts04']['Are_you_interested_in_an_indiv'] = None
        #             rhs_record['group_oi8ts04']['What_kind_of_toilet_would_you_like'] = None
        #             rhs_record['group_oi8ts04']['If_yes_why'] = None
        ...
        #         rhs_record['group_oi8ts04']['If_yes_why'] = None
        #         rhs_record['group_oi8ts04']['If_no_why'] = None
        #     if rhs_record['group_oi8ts04']['C2'] == "09":
        ...
        #
        #
        #     if rhs_record['group_oi8ts04']['When_did_you_receive_r_second_installment'] and not rhs_record['group_oi8ts04']['When_did_you_receive_ur_first_installment']:
        #         rhs_record['group_oi8ts04']['If_no_why'] = None
        #         rhs_record['group_oi8ts04']['When_did_you_receive_r_second_installment'] = None
        #
        ...
        #         if not rhs_record['group_oi8ts04']['Status_of_toilet_under_SBM']:
        #             if rhs_record['group_oi8ts04']['Current_place_of_defecation'] == "05":
        #                 if not rhs_record['group_oi8ts04']['C2']:
        #                     rhs_record['group_oi8ts04']['C2']="05"
        #                 rhs_record['group_oi8ts04']['Have_you_applied_for_individua'] = "03"
        #                 rhs_record['group_oi8ts04']['Type_of_SBM_toilets'] = None
        #                 rhs_record['group_oi8ts04']['How_many_installments_have_you'] = None
        #             if rhs_record['group_oi8ts04']['Current_place_of_defecation'] in ["09","10"]:
        #                 rhs_record['group_oi8ts04']['Status_of_toilet_under_SBM'] = "05"
        #                 if not rhs_record['group_oi8ts04']['C2']:
        #                     rhs_record['group_oi8ts04']['C2'] = "08"
        #                     rhs_record['group_oi8ts04']['C3'] = rhs_record['group_oi8ts04']['Current_place_of_defecation']
        #
        #
        #
        # if not rhs_record['group_oi8ts04']['OD1']:
        #     if rhs_record['group_oi8ts04']['Current_place_of_defecation'] == "12":
        #         rhs_record['group_oi8ts04']['OD1'] = "05"
        #     if rhs_record['group_oi8ts04']['Current_place_of_defecation'] in ["05","09","10"]:
        #         rhs_record['group_oi8ts04']['OD1'] = "01"
        ...
        if rhs_record["group_oi8ts04"]["C3"]:
            org_curr = rhs_record["group_oi8ts04"]["C3"]
        if rhs_record["group_oi8ts04"]["C4"]:
            org_curr = rhs_record["group_oi8ts04"]["C4"]
        if rhs_record["group_oi8ts04"]["C5"]:
            org_curr = rhs_record["group_oi8ts04"]["C5"]
        if not org_curr or org_curr.strip() == "":
            print("Hello")
...
def create_xml_string(
    xml_dict, repeat_dict, xml_root, xml_root_attr_id, xml_root_attr_version
):
        ...
        for key, val in repeat_dict.items():
            if val["list"]:
                ...
                for sub_xml_dict in val["list"]:
                    sub_xml_string = dicttoxml.dicttoxml(
                        sub_xml_dict, attr_type=False, root=False
                    )
