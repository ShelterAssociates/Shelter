...
def fetch_db_records(query):
    ...
    conn = psycopg2.connect(
        database=db_name, user=db_user, password=db_pwd, host=db_host, port=db_port
    )
    ...
    for row in range(sheet_survey.nrows):
        if row != 0:
            ...
            if (
                ...
                and name not in question_map_dict
            ):
        ...
        if not (question_id is None):
            ...
            if not (old_option is None):
                ...
                if isinstance(old_option, int):
                    question_option_map_dict[question_id].setdefault(
...
def get_approximat_huts(slum_id, answer):
    ...
    qry_ra_slum_household_count = (
        "select count(id) from slum_data_household where slum_id=%s "
    )
...
def write_log(msg):
    ...
    log_path = (
        options_dict["log_folder_path"]
        if options_dict["log_folder_path"]
        else log_folder_path
    )

    log_file = os.path.join(
        log_path, "log_" + str(strftime("%d_%m_%Y", gmtime())) + ".txt"
    )
            ...
            if download_folder_path:
                ...
                try:
                    ...
                    shutil.copy(
                        os.path.join(
                            root_folder_path,
                            "../../backup/shelter_survey/shelter_survey/static",
                            photo_path,
                        ),
                        download_photo_path,
                    )
                    ...
                    exception_log = (
                        "Exception occurred for fetching photo \t  exception : "
                        + str(ex)
                        + " \t  traceback : "
...
def show_progress_bar(iteration, total_count, status_for=""):
    ...
    complete_percent = ("{0:." + str(decimal_length) + "f}").format(
        100 * (iteration / float(total_count))
    )

    progress_bar_length = int(max_length * iteration // total_count)

    progress_bar = progress_char * progress_bar_length + " " * (
        max_length - progress_bar_length
    )
...
def show_process_status():
    if process_status["slum"]:
        ...
        slum_status += (
            ""
            if process_status["slum_unprocess"] == 0
            else "\t total slums unable to process "
            + str(process_status["slum_unprocess"])
        )
    ...
    if process_status["household"]:
        household_status = "Total household in all slums : " + str(
            process_status["household"]
        )
        household_status += (
            ""
            if process_status["household_unprocess"] == 0
            else "\t total household unable to process "
            + str(process_status["household_unprocess"])
        )
                    ...
                    for value in xml_root.findall(element):
                        ...
                        if photo_name:
                            photo_file = os.path.join(
                                base_folder, "photos", slum_folder, photo_name
                            )
