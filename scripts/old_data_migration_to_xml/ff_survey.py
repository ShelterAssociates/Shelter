...
qry_ff_slum_household_survey_list = (
...
def create_ff_xml(options):
    ...
    slum_household_list = get_household_survey(
        qry_ff_slum_household_survey_list % (survey_id, project_id)
    )
    ...
    for slum, household_list in slum_household_list.items():
        ...
        if slum_code:
            ...
            household_fact = get_household_wise_question_answer(
                qry_ff_survey_slum_household_question_answer
                % (survey_id, project_id, slum, survey_id, project_id, slum)
            )
            # print(household_fact)
            total_process_house += len(household_fact)

            household_photo_fact = get_household_wise_question_answer(
                qry_ff_survey_slum_household_photos % (survey_id, project_id, slum)
            )
            ...
            for household in household_list:
                ...
                try:
                    ...
                    ff_xml_dict["group_vq77l17"]["Settlement_address"] = get_answer(
                        "Settlement_address", fact
                    )

                    Household_number = get_answer("Household_number", fact)
                    try:
                        ff_xml_dict["group_vq77l17"]["Household_number"] = int(
                            Household_number
                        )
                    except:
                    ...
                    # Family Information
                    ff_xml_dict["group_oh4zf84"]["Name_of_the_family_head"] = (
                        get_answer("Name_of_the_family_head", fact)
                    )
                    ff_xml_dict["group_oh4zf84"][
                        "Name_of_Native_villa_district_and_state"
                    ] = get_answer("Name_of_Native_villa_district_and_state", fact)
                    ff_xml_dict["group_oh4zf84"][
                        "Duration_of_stay_in_the_city_in_Years"
                    ] = get_answer("Duration_of_stay_in_the_city_in_Years", fact)
                    ff_xml_dict["group_oh4zf84"][
                        "Duration_of_stay_in_settlement_in_Years"
                    ] = get_answer("Duration_of_stay_in_settlement_in_Years", fact)
                    ...
                    try:
                        ff_xml_dict["group_im2th52"]["Total_family_members"] = int(
                    ...
                    try:
                        ff_xml_dict["group_im2th52"]["Number_of_Male_members"] = int(
                    ...
                    Number_of_Female_members = get_answer(
                        "Number_of_Female_members", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"]["Number_of_Female_members"] = int(
                    ...
                    Number_of_Children_under_5_years_of_age = get_answer(
                        "Number_of_Children_under_5_years_of_age", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"][
                            "Number_of_Children_under_5_years_of_age"
                        ] = int(Number_of_Children_under_5_years_of_age)
                    except:
                    ...
                    Number_of_members_over_60_years_of_age = get_answer(
                        "Number_of_members_over_60_years_of_age", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"][
                            "Number_of_members_over_60_years_of_age"
                        ] = int(Number_of_members_over_60_years_of_age)
                    except:
                    ...
                    Number_of_Girl_children_between_0_18_yrs = get_answer(
                        "Number_of_Girl_children_between_0_18_yrs", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"][
                            "Number_of_Girl_children_between_0_18_yrs"
                        ] = int(Number_of_Girl_children_between_0_18_yrs)
                    except:
                    ...
                    Number_of_disabled_members = get_answer(
                        "Number_of_disabled_members", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"]["Number_of_disabled_members"] = (
                            int(Number_of_disabled_members)
                        )
                        ...
                        if (
                            ...
                            ff_xml_dict["group_im2th52"][
                                "If_yes_specify_type_of_disability"
                            ] = get_option_text(
                                disability_option_list,
                                get_answer("If_yes_specify_type_of_disability", fact),
                            )
                    except:
                    ...
                    Number_of_earning_members = get_answer(
                        "Number_of_earning_members", fact
                    )
                    try:
                        ff_xml_dict["group_im2th52"]["Number_of_earning_members"] = int(
                    ...
                    ff_xml_dict["group_im2th52"]["Occupation_s_of_earning_membe"] = (
                        get_answer("Occupation_s_of_earning_membe", fact)
                    )

                    ff_xml_dict["group_im2th52"][
                        "Approximate_monthly_family_income_in_Rs"
                    ] = get_answer("Approximate_monthly_family_income_in_Rs", fact)

                    # print('process - Family Members Information')
                    # write_log('process - Family Members Information')

                    # Toilet Information
                    Where_the_individual_ilet_is_connected_to = get_answer(
                        "Where_the_individual_ilet_is_connected_to", fact
                    )
                    if Where_the_individual_ilet_is_connected_to:
                        ff_xml_dict["group_ne3ao98"][
                            "Where_the_individual_ilet_is_connected_to"
                        ] = Where_the_individual_ilet_is_connected_to

                    Who_has_built_your_toilet = get_answer(
                        "Who_has_built_your_toilet", fact
                    )
                    if Who_has_built_your_toilet:
                        ff_xml_dict["group_ne3ao98"][
                            "Who_has_built_your_toilet"
                        ] = Who_has_built_your_toilet

                    Have_you_upgraded_yo_ng_individual_toilet = get_answer(
                        "Have_you_upgraded_yo_ng_individual_toilet", fact
                    )
                    if Have_you_upgraded_yo_ng_individual_toilet:
                        ff_xml_dict["group_ne3ao98"][
                            "Have_you_upgraded_yo_ng_individual_toilet"
                        ] = Have_you_upgraded_yo_ng_individual_toilet

                    ff_xml_dict["group_ne3ao98"]["Cost_of_upgradation_in_Rs"] = (
                        get_answer("Cost_of_upgradation_in_Rs", fact)
                    )
                    ...
                    photo_downlaod_folder = os.path.join(
                        output_folder_path, "photos", "slum_" + str(slum)
                    )
                    # print ("hello",photo_fact)
                    ff_xml_dict["Family_Photo"] = get_ff_photo(
                        "Family_Photo", photo_fact, photo_downlaod_folder
                    )
                    ff_xml_dict["Toilet_Photo"] = get_ff_photo(
                        "Toilet_Photo", photo_fact, photo_downlaod_folder
                    )
                    ...
                    # create xml file
                    file_name = (
                        "FF_Survey_Slum_Id_" + str(slum) + "_House_code_" + household
                    )
