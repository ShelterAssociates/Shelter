...
qry_rhs_slum_household_survey_list = (
    "select distinct household.slum_id, household.household_code from survey_fact f \
...
qry_rhs_master_slum_household_survey_list = (
    "select distinct household.slum_id, household.household_code from survey_fact f \
join slum_data_household household on household.id = f.object_id \
join survey_survey s on s.id = f.survey_id join survey_project p on p.id = s.project_id \
where s.id = %s and p.id = %s and f.content_type_id = 27 \
...
def create_rhs_xml(options):
    ...
    if survey_id2:
        ...
        common_slum_id_list = get_list_ids(
            qry_rhs_common_slum_list % (survey_id, project_id, survey_id2, project_id)
        )
        write_log(
            "fatch common slum in both data for RHS -- "
            + (", ".join(str(x) for x in common_slum_id_list))
        )

        # print('common_slum_id_list=> ', common_slum_id_list)

        master_slum_household_list = get_household_survey(
            qry_rhs_master_slum_household_survey_list % (survey_id2, project_id)
        )
    ...
    new_slum_household_list = get_household_survey(
        qry_rhs_slum_household_survey_list % (survey_id, project_id)
    )
        ...
        if slum_household_list:
            ...
            for slum, household_list in slum_household_list.items():
                ...
                if slum_code:
                    ...
                    if rhs_key == "master":
                        qry_rhs_question_answer = (
                            qry_rhs_master_survey_slum_household_question_answer
                            % (
                                survey_id2,
                                project_id,
                                slum,
                    ...
                    for household in household_list:
                        ...
                        try:
                            ...
                            if date_of_rhs:
                                rhs_xml_dict["group_ce0hf58"]["date_of_rhs"] = (
                                    get_formatted_data(date_of_rhs)
                                )

                            rhs_xml_dict["group_ce0hf58"][
                                "name_of_surveyor_who_collected_rhs_data"
                            ] = get_answer(
                                "name_of_surveyor_who_collected_rhs_data", fact
                            )
                            rhs_xml_dict["group_ce0hf58"]["house_no"] = get_answer(
                                "house_no", fact
                            )

                            Type_of_structure_occupancy = get_answer(
                                "Type_of_structure_occupancy", fact
                            )
                            ...
                            if (
                                Type_of_structure_occupancy == "01"
                                or Type_of_structure_occupancy == "03"
                            ):
                                rhs_xml_dict["group_ye18c77"]["group_ud4em45"][
                                    "what_is_the_full_name_of_the_family_head_"
                                ] = get_answer(
                                    "what_is_the_full_name_of_the_family_head_", fact
                                )
                                rhs_xml_dict["group_ye18c77"]["group_ud4em45"][
                                    "mobile_number"
                                ] = get_answer("mobile_number", fact)
                                rhs_xml_dict["group_ye18c77"]["group_ud4em45"][
                                    "adhar_card_number"
                                ] = get_answer("adhar_card_number", fact)
                            ...
                            if Type_of_structure_occupancy == "01":
                                what_is_the_structure_of_the_house = get_answer(
                                    "what_is_the_structure_of_the_house", fact
                                )
                                if what_is_the_structure_of_the_house:
                                    rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                        "what_is_the_structure_of_the_house"
                                    ] = what_is_the_structure_of_the_house

                                what_is_the_ownership_status_of_the_house = get_answer(
                                    "what_is_the_ownership_status_of_the_house", fact
                                )
                                if what_is_the_ownership_status_of_the_house:
                                    rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                        "what_is_the_ownership_status_of_the_house"
                                    ] = what_is_the_ownership_status_of_the_house

                                number_of_family_members = get_answer(
                                    "number_of_family_members", fact
                                )
                                try:
                                    rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                        "number_of_family_members"
                                    ] = get_rhs_family_member_count(
                                        number_of_family_members
                                    )
                                except:
                                ...
                                Do_you_have_a_girl_child_under = get_answer(
                                    "Do_you_have_a_girl_child_under", fact
                                )
                                if Do_you_have_a_girl_child_under:
                                    ...
                                    if Do_you_have_a_girl_child_under == "01":
                                        rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                            "if_yes_how_many_"
                                        ] = int(get_answer("if_yes_how_many_", fact))

                                house_area_in_sq_ft = get_answer(
                                    "house_area_in_sq_ft", fact
                                )
                                try:
                                    rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                        "house_area_in_sq_ft"
                                    ] = get_rhs_area_in_squar_feet(house_area_in_sq_ft)
                                except:
                                ...
                                Current_place_of_defecation_toilet = get_answer(
                                    "Current_place_of_defecation_toilet", fact
                                )
                                ...
                                does_any_member_of_your_family_go_for_open_defecation_ = get_answer(
                                    "does_any_member_of_your_family_go_for_open_defecation_",
                                    fact,
                                )
                                ...
                                if Current_place_of_defecation_toilet and (
                                    Current_place_of_defecation_toilet == "01"
                                    or Current_place_of_defecation_toilet == "02"
                                ):
                                    where_the_individual_toilet_is_connected_to_ = get_answer(
                                        "where_the_individual_toilet_is_connected_to_",
                                        fact,
                                    )
                                ...
                                type_of_water_connection = get_answer(
                                    "type_of_water_connection", fact
                                )
                                ...
                                if Current_place_of_defecation_toilet and (
                                    ...
                                    Are_you_interested_in_individu = get_answer(
                                        "Are_you_interested_in_individu", fact
                                    )
                                    if Are_you_interested_in_individu:
                                        ...
                                        if Are_you_interested_in_individu == "01":
                                            if_yes_why_ = get_answer(
                                                "if_yes_why_", fact
                                            )
                                            if if_yes_why_:
                                                rhs_xml_dict["group_ye18c77"][
                                                    "group_yw8pj39"
                                                ]["if_yes_why_"] = if_yes_why_
                                        ...
                                        if Are_you_interested_in_individu == "02":
                                            ...
                                            if if_no_why_:
                                                rhs_xml_dict["group_ye18c77"][
                                                    "group_yw8pj39"
                                                ]["if_no_why_"] = if_no_why_

                                        if (
                                            Are_you_interested_in_individu == "01"
                                            and city != 3789
                                        ):
                                            type_of_toilet_preference = get_answer(
                                                "type_of_toilet_preference", fact
                                            )
                                ...
                                if (
                                    Current_place_of_defecation_toilet
                                    and Current_place_of_defecation_toilet != "01"
                                ):
                                    Have_you_applied_for_indiviual = get_answer(
                                        "Have_you_applied_for_indiviual", fact
                                    )
                                    if Have_you_applied_for_indiviual:
                                        rhs_xml_dict["group_ye18c77"]["grup_yw8pj39"][
                                            "Have_you_applied_for_indiviual"
                                        ] = Have_you_applied_for_indiviual

                                        if Have_you_applied_for_indiviual == "01":
                                            How_many_installements_have_yo = get_answer(
                                                "How_many_installements_have_yo", fact
                                            )
                                            if How_many_installements_have_yo:
                                                rhs_xml_dict["group_ye18c77"][
                                                    "group_yw8pj39"
                                                ][
                                                    "How_many_installements_have_yo"
                                                ] = How_many_installements_have_yo

                                                if (
                                                    How_many_installements_have_yo
                                                    == "02"
                                                    or How_many_installements_have_yo
                                                    == "03"
                                                ):
                                                    rhs_xml_dict["group_ye18c77"][
                                                        "group_yw8pj39"
                                                    ][
                                                        "when_did_you_receive_the_first_installment_date"
                                                    ] = get_answer(
                                                        "when_did_you_receive_the_first_installment_date",
                                                        fact,
                                                    )
                                                ...
                                                if (
                                                    ...
                                                    rhs_xml_dict["group_ye18c77"][
                                                        ...
                                                        "when_did_you_receive_the_second_installment_date"
                                                    ] = get_answer(
                                                        "when_did_you_receive_the_second_installment_date",
                                                        fact,
                                                    )

                                            what_is_the_status_of_toilet_under_sbm_ = get_answer(
                                                "what_is_the_status_of_toilet_under_sbm_",
                                                fact,
                                            )
                                ...
                                Does_any_family_members_has_co = get_answer(
                                    "Does_any_family_members_has_co", fact
                                )
                                if Does_any_family_members_has_co:
                                    rhs_xml_dict["group_ye18c77"]["group_yw8pj39"][
                                        "Does_any_family_members_has_co"
                                    ] = Does_any_family_members_has_co
                            ...
                            rhs_xml_dict["meta"]["instanceID"] = "uuid:" + str(
                                uuid.uuid4()
                            )
                            ...
                            final_output_folder_path = os.path.join(
                                output_folder_path, "slum_" + str(slum)
                            )

                            create_xml_file(
                                xml_root_string, file_name, final_output_folder_path
                            )
                            ...
                            exception_log = (
                                "Exception occurred for household id "
                                + str(household)
                                + " of slum id "
                                + str(slum)
                                + " \t  exception : "
                            ...
                            unprocess_records[str(slum)].append(
                                [str(household), str(ex)]
                            )
                    ...
                    unprocess_records[str(slum)].append(
                        [None, "slum code is not found when mapped"]
                    )
            # break;
