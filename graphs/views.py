...
CARDS = {
    "Cards": {
        "General": [
            {"slum_count": "Slum count"},
            {"occupied_household_count": "Occupied household count"},
            {"gen_avg_household_size": "Avg Family size"},
            {"gen_tenement_density": "Tenement density (Huts/Hector)"},
            {"household_owners_count": "Superstructure Ownership"},
            {"type_of_structure_of_the_house": "Kutcha | Pucca | Semi-Pucca (%)"},
        ],
        "Waste": [
            {"waste_no_collection_facility_percentile": "Garbage Bin"},
            {
                "waste_door_to_door_collection_facility_percentile": "Door to door waste collection"
            },
            {"waste_dump_in_open_percent": "Dump in open"},
            {"drains_coverage": "ULB Service"},
            {"waste_other_services": "Other services"},
        ],
        "Water": [
            {"water_individual_connection_percentile": "Individual water connection"},
            {"water_shared_service_percentile": "Shared Water Connection"},
            {"waterstandpost_percentile": "Water Standposts"},
            {"water_other_services": "Other sources"},
        ],
        "Toilet": [
            {"toilet_seat_to_person_ratio": "Toilet seat to person ratio"},
            {"toilet_men_women_seats_ratio": "Men | Women | Mix toilet seats"},
            {"individual_toilet_coverage": "Individual Toilet Coverage"},
            {"ctb_coverage": "CTB usage"},
            {"shared_group_toilet_coverage": "Shared/Group Toilet usage"},
            {"other_usage": "Other usage"},
        ],
        "Road": [
            {"road_with_no_vehicle_access": "No. of slums with no vehicle access"},
            {"pucca_road_coverage": "Pucca Road Coverage"},
            {"kutcha_road_coverage": "Kutcha Road Coverage"},
        ],
    ...
    "Keytakeaways": {
        "General": [
            [
                " <b>value</b> slum/s with <b>undeclared</b> land status",
            ],
            [
        ...
        "Water": [
            [
                "<b>value</b> slum/s where water availability  is <b>less than 2 hours</b>, "
            ],
        ...
        "Waste": [
            [" <b>value open community dumping sites</b> present in slum/s"],
        ...
        "Toilet": [
            [" <b>value</b>  slum/s where CTB structure is in <b> poor </b> condition"],
...
def get_dashboard_card(request, key):
    ...
    # City level
    output_data["city"][city.name.city_name] = {
        "scores": {},
        "cards": {},
    }  # ,'key_takeaways':{} }
    list_associated_with_SA_city = Slum.objects.filter(
        associated_with_SA=True,
        electoral_ward__administrative_ward__city__name__city_name=city,
    ).exclude(status=False)
    qol_scores = QOLScoreData.objects.filter(city=city)
    for clause in select_clause:
        output_data["city"][city.name.city_name]["scores"][clause] = (
    ...
    for admin_ward in AdministrativeWard.objects.filter(city=city):
        output_data["administrative_ward"][admin_ward.name] = {
            "scores": {},
            "cards": {},
        }  # , 'key_takeaways':{}}
        qol_scores = QOLScoreData.objects.filter(
            slum__electoral_ward__administrative_ward=admin_ward
        )
        list_associated_with_SA_admins = Slum.objects.filter(
            associated_with_SA=True, electoral_ward__administrative_ward=admin_ward
        )
        for clause in select_clause:
            output_data["administrative_ward"][admin_ward.name]["scores"][clause] = (
                qol_scores.aggregate(Avg(clause))[clause + "__avg"]
            )
        admin_wards = DashboardData.objects.filter(
    ...
    for electoral_ward in ElectoralWard.objects.filter(administrative_ward__city=city):
        output_data["electoral_ward"][electoral_ward.name] = {
            "scores": {},
            "cards": {},
        }  # , 'key_takeaways':{}}
        qol_scores = QOLScoreData.objects.filter(slum__electoral_ward=electoral_ward)
        list_associated_with_SA_electorals = Slum.objects.filter(
            associated_with_SA=True, electoral_ward=electoral_ward
        )
        for clause in select_clause:
            output_data["electoral_ward"][electoral_ward.name]["scores"][clause] = (
                qol_scores.aggregate(Avg(clause))[clause + "__avg"]
            )
        ele_wards = DashboardData.objects.filter(
    ...
    qol_scores = QOLScoreData.objects.filter(
        slum__in=list_associated_with_SA_city
    ).values(*select_clause)
    qol_scores = groupby(qol_scores, key=lambda x: x["slum__name"])
    qol_scores = {
        key: {
            "scores": list(values)[0],
            "cards": score_cards(DashboardData.objects.filter(slum__name=key)),
            "key_takeaways": all_key_takeaways(key),
        }
        for key, values in qol_scores
    }
...
def score_cards(ele):
    ...
    aggrgated_data = ele.aggregate(
        Sum("occupied_household_count"),
        Sum("kutcha_household_cnt"),
        Sum("puccha_household_cnt"),
        Sum("semi_puccha_household_cnt"),
        Sum("household_count"),
        Sum("total_road_area"),
        Sum("road_with_no_vehicle_access"),
        Sum("pucca_road_coverage"),
        Sum("kutcha_road_coverage"),
        Sum("gen_avg_household_size"),
        Sum("gen_tenement_density"),
        Sum("household_owners_count"),
        Sum("waste_no_collection_facility_percentile"),
        Sum("waste_door_to_door_collection_facility_percentile"),
        Sum("waste_dump_in_open_percent"),
        Sum("water_other_services"),
        Sum("waste_other_services"),
        Sum("water_individual_connection_percentile"),
        Sum("water_shared_service_percentile"),
        Sum("waterstandpost_percentile"),
        Sum("toilet_seat_to_person_ratio"),
        Sum("individual_toilet_coverage"),
        Sum("fun_male_seats"),
        Sum("fun_fmale_seats"),
        Sum("toilet_men_women_seats_ratio"),
        Sum("ctb_coverage"),
        Sum("get_shops_count"),
        Sum("drains_coverage"),
        Sum("shared_group_toilet_coverage"),
        Sum("other_services_toilet_coverage"),
        Sum("toilet_data_available"),
        Sum("water_data_available"),
        Sum("waste_data_available"),
    )

    # drainage_card data
    slum_ids_hh = ele.filter(household_count__gt=0.0).values_list("slum_id", flat=True)
    total_drain_count = 0
    data = Rapid_Slum_Appraisal.objects.filter(slum_name__id__in=slum_ids_hh).aggregate(
        Sum("drainage_coverage")
    )
    total_drain_count += (
        data["drainage_coverage__sum"] if data["drainage_coverage__sum"] != None else 0
    )
            ...
            for k, v in v1.items():
                ...
                if k == "Drainage":
                        ...
                        str(
                            round(
                                (
                                    (
                                        total_drain_count
                                        / aggrgated_data["household_count__sum"]
                                    )
                                    * 100
                                    if aggrgated_data["household_count__sum"]
                                    else 0
                                ),
                                2,
                            )
                    ...
                    cards[k].append(
                        str(
                            int(
                                aggrgated_data["road_with_no_vehicle_access__sum"]
                                if aggrgated_data["road_with_no_vehicle_access__sum"]
                                else 0
                            )
                        )
                    )
                    cards[k].append(
                        str(
                            round(
                                (
                                    (
                                        aggrgated_data["pucca_road_coverage__sum"]
                                        / aggrgated_data["total_road_area__sum"]
                                    )
                                    * 100
                                    if aggrgated_data["total_road_area__sum"]
                                    else 0
                                ),
                                2,
                            )
                    ...
                    cards[k].append(
                        str(
                            round(
                                (
                                    (
                                        aggrgated_data["kutcha_road_coverage__sum"]
                                        / aggrgated_data["total_road_area__sum"]
                                    )
                                    * 100
                                    if aggrgated_data["total_road_area__sum"]
                    ...
                    only_ctb_user = (
                        ...
                        if aggrgated_data["ctb_coverage__sum"]
                        else 0
                    )
                    ctb_seat_ratio = "1:" + str(
                        int(
                            (only_ctb_user * 5)
                            / aggrgated_data["toilet_seat_to_person_ratio__sum"]
                            if aggrgated_data["toilet_seat_to_person_ratio__sum"]
                            else 0
                        )
                    )
                    ...
                    mix = (
                        str(int(aggrgated_data["toilet_men_women_seats_ratio__sum"]))
                        if aggrgated_data["toilet_men_women_seats_ratio__sum"]
                        else "0"
                    )
                    men_wmn_seats_ratio = (
                        str(aggrgated_data["fun_male_seats__sum"])
                        + "|"
                        + str(aggrgated_data["fun_fmale_seats__sum"])
                        + "|"
                        + mix
                    )
                    ...
                    cards[k].append(
                        str(
                            round(
                                aggrgated_data["occupied_household_count__sum"]
                                if aggrgated_data["occupied_household_count__sum"]
                                else 0
                            )
                        )
                    )
                    ...
                    shops = (
                        aggrgated_data["get_shops_count__sum"]
                        if aggrgated_data["get_shops_count__sum"]
                        else 0
                    )  # this column contains shops count for slum
                    structure_fields = [
                        "kutcha_household_cnt__sum",
                        "puccha_household_cnt__sum",
                        "semi_puccha_household_cnt__sum",
                    ]
                    structure_total = sum(
                        [
                            aggrgated_data[field] if aggrgated_data[field] else 0
                            for field in structure_fields
                        ]
                    )
                    structure_str = [
                        (
                            str(round(aggrgated_data[field] / structure_total * 100, 2))
                            if aggrgated_data[field]
                            else str(0)
                        )
                        for field in structure_fields
                    ]
                    cards[k].append(
                        str(
                            round(
                                (
                                    aggrgated_data["gen_avg_household_size__sum"]
                                    / aggrgated_data["occupied_household_count__sum"]
                                    if aggrgated_data["occupied_household_count__sum"]
                                    else 0
                                ),
                    ...
                    cards[k].append(
                        str(
                            int(
                                (houses + shops)
                                / aggrgated_data["gen_tenement_density__sum"]
                                if aggrgated_data["gen_tenement_density__sum"]
                                else 0
                            )
                        )
                    )
                    cards[k].append(
                        str(
                            round(
                                (
                                    (
                                        aggrgated_data["household_owners_count__sum"]
                                        / aggrgated_data[
                                            "occupied_household_count__sum"
                                        ]
                                    )
                                    * 100
                                    if aggrgated_data["occupied_household_count__sum"]
                    ...
                    cards[k].append(
                        str(waste_door_to_door_collection_facility_percentile) + " %"
                    )
...
def key_takeaways_toilet(slum):
    ...
    for i in slum:
        ...
        safe_m2 += (
            int(i.seats_in_good_condtn) if type(i.seats_in_good_condtn) == int else 0
        )
        ...
        ctb_at_night += (
            1 if i.ctb_available_at_night in ["No", "Yes, but Not all night"] else 0
        )
...
def key_takeaways(slum_name):
    ...
    availability = slum_name.values("availability_of_water").annotate(
        count=Count("availability_of_water")
    )
    for i in availability:
        water_availability[i["availability_of_water"]] = i["count"]
    water_availability_2hrs = (
        water_availability["Less than 2 hrs"]
        if "Less than 2 hrs" in water_availability.keys()
        else 0
    )
    water_availability_24_7 = (
        water_availability["24/7"] if "Daily" in water_availability.keys() else 0
    )

    quality = slum_name.values("quality_of_water").annotate(
        count=Count("quality_of_water")
    )
    for i in quality:
    ...
    coverage = slum_name.values("coverage_of_water").annotate(
        count=Count("coverage_of_water")
    )
    for i in coverage:
        water_coverage[i["coverage_of_water"]] = i["count"]
    coverage_full = (
        water_coverage["Full coverage"]
        if "Full coverage" in water_coverage.keys()
        else 0
    )
    prtial_coverage = (
        water_coverage["Partial coverage"]
        if "Partial coverage" in water_coverage.keys()
        else 0
    )

    source = slum_name.values("alternative_source_of_water").annotate(
        count=Count("alternative_source_of_water")
    )
    for i in source:
        water_source[i["alternative_source_of_water"]] = i["count"]
    tanker = water_source["Tanker"] if "Tanker" in water_source.keys() else 0
    standpost = (
        water_source["Water standpost"]
        if "Water standpost" in water_source.keys()
        else 0
    )
    ...
    try:
        container = int(
            slum_name.values_list("number_of_waste_container", flat=True)[0]
        )
    except:
        container = 0
    count_waste_container += container

    cvrg_door_t_door = slum_name.values("waste_coverage_door_to_door").annotate(
        count=Count("waste_coverage_door_to_door")
    )
    for i in cvrg_door_t_door:
        waste_coverage_door[i["waste_coverage_door_to_door"]] = i["count"]
    full_coverage = (
        waste_coverage_door["Full coverage"]
        if "Full coverage" in waste_coverage_door.keys()
        else 0
    )
    partial_coverage = (
        waste_coverage_door["Partial coverage"]
        if "Partial coverage" in waste_coverage_door.keys()
        else 0
    )
    ...
    freq_door = slum_name.values("waste_coll_freq_door_to_door").annotate(
        count=Count("waste_coll_freq_door_to_door")
    )
    for i in freq_door:
        waste_freq_door[i["waste_coll_freq_door_to_door"]] = i["count"]
    door_freq = (
        waste_freq_door["Less than twice a week"]
        if "Less than twice a week" in waste_freq_door.keys()
        else 0
    )
    door_freq_daily = (
        waste_freq_door["Daily"] if "Daily" in waste_freq_door.keys() else 0
    )

    freq_van = slum_name.values("waste_coll_freq_by_ulb_van").annotate(
        count=Count("waste_coll_freq_by_ulb_van")
    )
    for i in freq_van:
        waste_freq_van[i["waste_coll_freq_by_ulb_van"]] = i["count"]
    van_freq = (
        waste_freq_van["Less than twice a week"]
        if "Less than twice a week" in waste_freq_van.keys()
        else 0
    )
    van_freq_daily = waste_freq_van["Daily"] if "Daily" in waste_freq_van.keys() else 0

    freq_tempo = slum_name.values("waste_coll_freq_by_mla_tempo").annotate(
        count=Count("waste_coll_freq_by_mla_tempo")
    )
    for i in freq_tempo:
        waste_freq_tempo[i["waste_coll_freq_by_mla_tempo"]] = i["count"]
    tempo_freq = (
        waste_freq_tempo["Less than twice a week"]
        if "Less than twice a week" in waste_freq_tempo.keys()
        else 0
    )
    tempo_freq_daily = (
        waste_freq_tempo["Daily"] if "Daily" in waste_freq_tempo.keys() else 0
    )

    freq_ghantagadi = slum_name.values("waste_coll_freq_by_ghantagadi").annotate(
        count=Count("waste_coll_freq_by_ghantagadi")
    )
    for i in freq_ghantagadi:
        waste_freq_ghantagadi[i["waste_coll_freq_by_ghantagadi"]] = i["count"]
    gadi_freq = (
        waste_freq_ghantagadi["Less than twice a week"]
        if "Less than twice a week" in waste_freq_ghantagadi.keys()
        else 0
    )
    gadi_freq_daily = (
        waste_freq_ghantagadi["Daily"] if "Daily" in waste_freq_ghantagadi.keys() else 0
    )

    freq_colln_twice = door_freq + van_freq + gadi_freq + tempo_freq
    freq_colln_daily = (
        gadi_freq_daily + tempo_freq_daily + door_freq_daily + van_freq_daily
    )
    ...
    # drainage data
    drain_gradient_adequete = slum_name.filter(
...
def dashboard_all_cards(request, key):
    ...
    def get_data(key):
        ...
        for city in cities:
            dashboard_data = DashboardData.objects.filter(city=city).aggregate(
                Sum("slum_population"),
                Sum("household_count"),
                Sum("count_of_toilets_completed"),
                Sum("people_impacted"),
            )
            total_slum_count = Slum.objects.filter(
                electoral_ward__administrative_ward__city=city, associated_with_SA=True
            ).count()
            slum_count = (
                Slum.objects.filter(
                    electoral_ward__administrative_ward__city=city,
                    associated_with_SA=True,
                )
                .exclude(status=False)
                .count()
            )
            qol_scores = QOLScoreData.objects.filter(city=city).aggregate(
                Avg("totalscore_percentile")
            )
            ...
            if city_name in city_array:
                output_data["city"][city_name]["household_count__sum"] = city_array[
                    city_name
                ]
        return output_data
...
def give_report_covid_data(request):  # view for covid data
    ...
    for query_field in query_on.keys():
        ...
        tc = (
            CovidData.objects.filter(**filter_field)
            .exclude(household_number=9999)
            .distinct()
            .annotate(**level_data[tag])
            .values("level", "level_id", "city_name", "city")
        ...
        tm = (
            ...
            .exclude(household_number=9999)
            .annotate(**level_data[tag])
            .values("level_id", "household_number", "slum", "city")
            .distinct()
        )
        ...
        for id_ in level_ids:
            if tag == "city":
                ...
                th_ = house_data.filter(
                    slum__electoral_ward__administrative_ward__city__id=id_,
                ...
                dose_no = (
                    covid_data.filter(
                        Q(age__range=[12, 17])
                        & Q(slum__electoral_ward__administrative_ward__city__id=id_)
                        & (Q(take_first_dose="No") | Q(take_first_dose__isnull=True))
                        & Q(last_modified_date__range=[start_date, end_date])
                    )
                    .exclude(household_number=9999)
                    .count()
                )
                ...
                total_structure = house_data.filter(
                    slum__electoral_ward__administrative_ward__city__id=id_,
                    rhs_data__isnull=False,
                ).values_list("rhs_data", flat=True)
                total_struc = [
                    data["Type_of_structure_occupancy"]
                    for data in total_structure
                    if "Type_of_structure_occupancy" in data
                    and data["Type_of_structure_occupancy"] != "Shop"
                ]
                ...
                for k, v in hh_list.items():
                    hh_data1 = house_data.filter(
                        household_number__in=v, slum__id=k, rhs_data__isnull=False
                    ).values_list("rhs_data", flat=True)
                    occ_data = [
                        occ_cnt
                        for i in hh_data1
                        if (
                            i["Type_of_structure_occupancy"]
                            and i["Type_of_structure_occupancy"] == "Occupied house"
                        )
                    ]
                ...
                # For Total Surveyed Members
                th_ = house_data.filter(
                    slum__electoral_ward__administrative_ward__id=id_
                ).count()
                blw_18 = (
                    covid_data.filter(
                        age__lt=12, slum__electoral_ward__administrative_ward__id=id_
                    )
                    .exclude(
                        household_number=9999,
                        last_modified_date__range=[start_date, end_date],
                    )
                    .count()
                )
                ...
                dose_no = (
                    covid_data.filter(
                        Q(age__range=[12, 17])
                        & Q(slum__electoral_ward__administrative_ward__id=id_)
                        & (Q(take_first_dose="No") | Q(take_first_dose__isnull=True))
                        & Q(last_modified_date__range=[start_date, end_date])
                    )
                    .exclude(household_number=9999)
                ...
                blw_18 = (
                    covid_data.filter(age__lt=12, slum__electoral_ward__id=id_)
                    .exclude(
                        household_number=9999,
                        last_modified_date__range=[start_date, end_date],
                    )
                ...
                dose_no = (
                    covid_data.filter(
                        Q(age__range=[12, 17])
                        & Q(slum__electoral_ward__id=id_)
                        & (Q(take_first_dose="No") | Q(take_first_dose__isnull=True))
                        & Q(last_modified_date__range=[start_date, end_date])
                    )
                    .exclude(household_number=9999)
                ...
                dose_no = (
                    covid_data.filter(
                        ...
                        & Q(slum__electoral_ward__id=id_)
                        & (
                            Q(take_first_dose="No")
                            | Q(take_first_dose__isnull=True)
                            & Q(last_modified_date__range=[start_date, end_date])
                        )
                    )
                    .exclude(household_number=9999)
                ...
                total_structure = house_data.filter(
                    slum__electoral_ward__id=id_, rhs_data__isnull=False
                ).values_list("rhs_data", flat=True)
                total_struc = [
                    data["Type_of_structure_occupancy"]
                    for data in total_structure
                ...
                blw_18 = (
                    covid_data.filter(age__lt=12, slum__id=id_)
                    .exclude(
                        household_number=9999,
                        last_modified_date__range=[start_date, end_date],
                    )
                ...
                # Total Reach percentage.
                total_structure = house_data.filter(
                    slum__id=id_, rhs_data__isnull=False
                ).values_list("rhs_data", flat=True)
                total_struc = [
                    data["Type_of_structure_occupancy"]
                    for data in total_structure
...
def MemberDataView(request):
    ...
    member_data = list(
        MemberData.objects.filter(slum_id=slumname)
        .annotate(
            created_date_str=Cast("created_date", CharField()),
    ...
    if len(member_data) > 0:
        ...
        member_df["date_of_birth"] = pd.to_datetime(
            member_df["date_of_birth"], errors="coerce"
        )
        ...
        member_df["age"] = member_df["date_of_birth"].apply(
        ...
        member_df["gender"] = member_df["gender"].map(
