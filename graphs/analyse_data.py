...
class RHSData(object):
    ...
    def toilet_data_available(self):
        ...
        sanitation_hhs = filter(
            lambda x: x.rhs_data
            and "Type_of_structure_occupancy" in x.rhs_data
            and "group_oi8ts04/Current_place_of_defecation" in x.rhs_data,
            self.household_data,
        )
        ...
        followup_hhs = self.followup_data.filter(
            household_number__in=occupied_houses
    ...
    def water_data_available(self):
        ...
        water_hhs = filter(
            lambda x: x.rhs_data
            and "Type_of_structure_occupancy" in x.rhs_data
            and "group_el9cl08/Type_of_water_connection" in x.rhs_data,
            self.household_data,
        )
    ...
    def waste_data_available(self):
        ...
        waste_hhs = filter(
            lambda x: x.rhs_data
            and "Type_of_structure_occupancy" in x.rhs_data
            and "group_el9cl08/Facility_of_solid_waste_collection" in x.rhs_data,
            self.household_data,
        )
    ...
    def get_unique_houses_rhs_householddata(self):
        ...
        """
        unique_houses = self.household_data.distinct("household_number").values_list(
    ...
    def occupied_houses(self):
        ...
        occupied_hhs = filter(
            lambda x: x.rhs_data
            and "Type_of_structure_occupancy" in x.rhs_data
            and x.rhs_data["Type_of_structure_occupancy"] == "Occupied house",
            self.household_data,
        )
    ...
    def structure_data(self):
        ...
        for obj in self.household_data:
            if (
                obj.rhs_data
                and "Type_of_structure_occupancy" in obj.rhs_data
                and obj.rhs_data["Type_of_structure_occupancy"] == "Occupied house"
            ):
                rhs = obj.rhs_data
    ...
    def ownership_status(self):
        ...
        owner_count = filter(
            lambda x: x.rhs_data
            and "group_el9cl08/Ownership_status_of_the_house" in x.rhs_data
            and x.rhs_data["Type_of_structure_occupancy"] == "Occupied house"
            and x.rhs_data["group_el9cl08/Ownership_status_of_the_house"]
            == "Own house",
            self.household_data,
        )
    ...
    def toilet_constructed(self):
        ...
        households_with_toilets_completed = self.toilet_ms_data.filter(
            status="6"
        ).values_list("household_number", flat=True)
    ...
    def ctb_count(self):
        ...
        for house in self.hh_have_toilet_enc:
            try:
                latest_record = self.followup_data.filter(
                    household_number=house
                ).latest("submission_date")
                if latest_record.household_number not in toilet_completed and (
                    "group_oi8ts04/Current_place_of_defecation"
                    in latest_record.followup_data
                    and latest_record.followup_data[
                        "group_oi8ts04/Current_place_of_defecation"
                    ]
                    == "Use CTB"
                ):
    ...
    def individual_toilet(self):
        ...
        for house in self.hh_have_toilet_enc:
            try:
                ...
                if (
                    latest_record.household_number not in toilet_completed
                    and "group_oi8ts04/Current_place_of_defecation"
                    in latest_record.followup_data
                    and (
                        latest_record.followup_data[
                            "group_oi8ts04/Current_place_of_defecation"
                        ]
                    )
                    in toilet_by_other
    ...
    def get_toilet_data(self):
        ...
        if (
            "Toilet" in self.slum_data.rim_data
            and len(self.slum_data.rim_data["Toilet"]) > 0
        ):
        ...
        for i in toilet_data:
            wrk_male_seats += int(
                i["number_of_seats_allotted_to_me"]
                if "number_of_seats_allotted_to_me" in i
                else 0
            )
            wrk_fmale_seats += int(
                i["number_of_seats_allotted_to_wo"]
                if "number_of_seats_allotted_to_wo" in i
                else 0
            )
            wrk_mix_seats += int(
                i["total_number_of_mixed_seats_al"]
        ...
        total_toilet_seats = (
            wrk_male_seats + wrk_fmale_seats + wrk_mix_seats
        )  # considered total seats
    ...
    def get_road_vehicle_facility(self):
        ...
        """
        no_vehicle_access = (
            self.slum_data.rim_data["Road"]["point_of_vehicular_access_to_t"]
            if "point_of_vehicular_access_to_t" in self.slum_data.rim_data["Road"]
            else 0
        )
    ...
    def get_road_type(self):
        ...
        """
        road_type = (
            self.slum_data.rim_data["Road"]["type_of_roads_within_the_settl"]
            if "type_of_roads_within_the_settl" in self.slum_data.rim_data["Road"]
            else 0
        )
        return road_type
    ...
    def get_shop_count(self):
        ...
        """
        slum_type = ContentType.objects.get_for_model(Slum)
        get_shops_count_in_slum = len(
            Component.objects.filter(
                content_type=slum_type, object_id=self.slum.id, metadata__name="Shops"
            ).values("shape")
        )
            ...
            get_shops_count_in_slum = len(
                list(
                    filter(
                        lambda x: x.rhs_data
                        and "Type_of_structure_occupancy" in x.rhs_data
                        and x.rhs_data["Type_of_structure_occupancy"] == "Shop",
                        self.household_data,
                    )
                )
            )
    ...
    def get_waste_facility(self, facility):
        ...
        """
        waste_facility_count = filter(
            lambda x: x.rhs_data
            and "group_el9cl08/Facility_of_solid_waste_collection" in x.rhs_data
            and x.rhs_data["group_el9cl08/Facility_of_solid_waste_collection"]
            == facility,
            self.household_data,
        )
    ...
    def get_perc_of_waste_collection(self, facility="Door to door waste collection"):
        ...
        """
        percent = (
            (self.get_waste_facility(facility) / self.get_household_count()) * 100
            if self.get_household_count() != 0
            else 0
        )
        return percent
    ...
    def get_water_coverage(self, type):
        ...
        """
        water_facility_count = filter(
            lambda x: x.rhs_data
            and "group_el9cl08/Type_of_water_connection" in x.rhs_data
            and x.rhs_data["group_el9cl08/Type_of_water_connection"] == type,
            self.household_data,
        )
    ...
    def get_household_members(self):
        ...
        """
        household_member = filter(
            lambda x: x.rhs_data
            and "group_el9cl08/Number_of_household_members" in x.rhs_data
    ...
    def get_household_member_total(self):
        ...
        for hh_data in self.household_data:
            if (
                hh_data.rhs_data
                and hh_data.rhs_data != {}
                and hh_data.rhs_data != "None"
            ):
                rhs = hh_data.rhs_data
                if (
                    "group_el9cl08/Number_of_household_members" in rhs
                    and rhs["group_el9cl08/Number_of_household_members"]
                    and rhs["group_el9cl08/Number_of_household_members"] != "None"
                    and rhs["group_el9cl08/Number_of_household_members"]
                    == rhs["group_el9cl08/Number_of_household_members"]
                ):
                    try:
                        slum_population += int(
                            rhs["group_el9cl08/Number_of_household_members"]
                        ...
                        logger.error(
                            hh_data.id, rhs["group_el9cl08/Number_of_household_members"]
                        )
    ...
    def get_slum_area_size_in_hectors(self):
        ...
        """
        return (
            int(self.slum_data.rim_data["General"]["approximate_area_of_the_settle"])
            / 10000
            if "approximate_area_of_the_settle" in self.slum_data.rim_data["General"]
            else 0
        )
    ...
    def key_takeaways_ctb(self, ctb):
        ...
        if "is_the_CTB_in_use" in ctb and ctb["is_the_CTB_in_use"] == "Yes":
            electricity = (
                ctb["availability_of_electricity_in_001"]
                if "availability_of_electricity_in_001" in ctb
                else 0
            )
            sewage_system = (
                ctb["sewage_disposal_system"] if "sewage_disposal_system" in ctb else 0
            )
            ctb_available_at_night = (
                ctb["is_the_ctb_available_at_night"]
                if "is_the_ctb_available_at_night" in ctb
                else 0
            )
            ctb_cleaning = (
                ctb["cleanliness_of_the_ctb"] if "cleanliness_of_the_ctb" in ctb else 0
            )
            ctb_cleaning_freq = (
                ctb["frequency_of_ctb_cleaning_by_U"]
                if "frequency_of_ctb_cleaning_by_U" in ctb
                else 0
            )
            water_in_ctb = (
                ctb["availability_of_water_in_the_t"]
                if "availability_of_water_in_the_t" in ctb
                else 0
            )
            water_supply_type = (
                ctb["type_of_water_supply_in_ctb"]
                if "type_of_water_supply_in_ctb" in ctb
                else 0
            )
            ctb_door = (
                ctb["out_of_total_seats_no_of_doors_in_good_condition"]
                if "out_of_total_seats_no_of_doors_in_good_condition" in ctb
                else 0
            )
            ctb_condition = (
                ctb["condition_of_ctb_structure"]
                if "condition_of_ctb_structure" in ctb
                else 0
            )
            ctb_seats_good_condtn = (
                ctb["out_of_total_seats_no_of_pans_in_good_condition"]
                if "out_of_total_seats_no_of_pans_in_good_condition" in ctb
                else 0
            )
            ctb_tank_capacity = (
                ctb["capacity_of_ctb_water_tank_in"]
                if "capacity_of_ctb_water_tank_in" in ctb
                else 0
            )
            cost_per_use = (
                ctb["cost_of_pay_and_use_toilet_pe"]
                if "cost_of_pay_and_use_toilet_pe" in ctb
                else 0
            )
            ctb_caretaker = (
                ctb["is_there_a_caretaker_for_the_C"]
                if "is_there_a_caretaker_for_the_C" in ctb
                else 0
            )
            for_child = (
                ctb["facility_in_the_toilet_block_f"]
                if "facility_in_the_toilet_block_f" in ctb
                else 0
            )
            ctb_cond_for_child = (
                ctb["condition_of_facility_for_chil"]
                if "condition_of_facility_for_chil" in ctb
                else 0
            )
    ...
    def key_takeaways_road(self):
        ...
        slum_level = (
            road["is_the_settlement_below_or_abo"]
            if "is_the_settlement_below_or_abo" in road
            else 0
        )
        huts_level = (
            road["are_the_huts_below_or_above_th"]
            if "are_the_huts_below_or_above_th" in road
            else 0
        )
        vehicular_access = (
            road["point_of_vehicular_access_to_t"]
            if "point_of_vehicular_access_to_t" in road
            else 0
        )
    ...
    def key_takeaways_water(self):
        ...
        water_availability = (
            water["availability_of_water"] if "availability_of_water" in water else 0
        )
        water_quality = (
            water["quality_of_water_in_the_system"]
            if "quality_of_water_in_the_system" in water
            else 0
        )
        water_coverage = (
            water["coverage_of_wateracross_settle"]
            if "coverage_of_wateracross_settle" in water
            else 0
        )
        alternate_water_source = (
            water["alternative_source_of_water"]
            if "alternative_source_of_water" in water
            else 0
        )
        return (
            water_availability,
    ...
    def key_takeaways_drain_n_gutter(self):
        ...
        drain_block = (
            drainage["do_the_drains_get_blocked"]
            if "do_the_drains_get_blocked" in drainage
            else 0
        )
        drain_gradient = (
            drainage["is_the_drainage_gradient_adequ"]
            if "is_the_drainage_gradient_adequ" in drainage
            else 0
        )
        gutter_flood = gutter["do_gutters_flood"] if "do_gutters_flood" in gutter else 0
        gutter_gradient = (
            gutter["is_gutter_gradient_adequate"]
            if "is_gutter_gradient_adequate" in gutter
            else 0
        )
    ...
    def key_takeaways_waste(self):
        ...
        dump_in_drains = (
            waste["do_the_member_of_community_dep"]
            if "do_the_member_of_community_dep" in waste
            else 0
        )
        community_dump_site = (
            waste["where_are_the_communty_open_du"]
            if "where_are_the_communty_open_du" in waste
            else 0
        )
        waste_containers = (
            waste["total_number_of_waste_containe"]
            if "total_number_of_waste_containe" in waste
            else 0
        )
        waste_coll_by_mla_tempo = (
            waste["coverage_of_waste_collection_a"]
            if "coverage_of_waste_collection_a" in waste
            else 0
        )
        waste_coll_door_to_door = (
            waste["coverage_of_waste_collection_a_001"]
            if "coverage_of_waste_collection_a_001" in waste
            else 0
        )
        waste_coll_by_ghantagadi = (
            waste["coverage_of_waste_collection_a_002"]
            if "coverage_of_waste_collection_a_002" in waste
            else 0
        )
        waste_coll_by_ulb_van = (
            waste["coverage_of_waste_collection_a_003"]
            if "coverage_of_waste_collection_a_003" in waste
            else 0
        )
        waste_freq_by_ulb_van = (
            waste["frequency_of_waste_collection_"]
            if "frequency_of_waste_collection_" in waste
            else 0
        )
        waste_freq_by_mla_tempo = (
            waste["frequency_of_waste_collection"]
            if "frequency_of_waste_collection" in waste
            else 0
        )
        waste_freq_door_to_door = (
            waste["frequency_of_waste_collection__002"]
            if "frequency_of_waste_collection__002" in waste
            else 0
        )
        waste_freq_by_ghantagadi = (
            waste["frequency_of_waste_collection_001"]
            if "frequency_of_waste_collection_001" in waste
            else 0
        )
        waste_freq_bin = (
            waste["frequency_of_waste_collection__001"]
            if "frequency_of_waste_collection__001" in waste
            else 0
        )
        return (
            dump_in_drains,
