...
class DashboardData(models.Model):
    ...
    gen_tenement_density = models.FloatField(
        blank=True, null=True
    )  # get_slum_area_size_in_hectors
    ...
    waste_no_collection_facility_percentile = models.FloatField(
        blank=True, null=True
    )  # repalce this field to garbage_bin_facility
    waste_door_to_door_collection_facility_percentile = models.FloatField(
        blank=True, null=True
    )
    ...
    # Toilet
    toilet_seat_to_person_ratio = models.FloatField(
        blank=True, null=True
    )  # total_toilet_seats
    fun_male_seats = models.IntegerField(
        blank=True, null=True
    )  # toilet seats in use (male)
    fun_fmale_seats = models.IntegerField(
        blank=True, null=True
    )  # toilet seats in use (female)
    toilet_men_women_seats_ratio = models.FloatField(
        blank=True, null=True
    )  # toilet seats in use (mix)
...
class CovidData(models.Model):
    ...
    surveyor_name = models.CharField(
        max_length=100, verbose_name="Name of the surveyor", null=True
    )
    ...
    family_member_name = models.CharField(
        max_length=100, verbose_name="Family member name", null=True
    )
    ...
    aadhar_number = models.CharField(
        max_length=100, verbose_name="Addhar number", null=True
    )
    do_you_have_any_other_disease = models.CharField(
        max_length=100, verbose_name="Do you have any other disease", null=True
    )
    if_any_then_which_disease = models.CharField(
        max_length=100, verbose_name="If any then which disease", null=True
    )
    preganant_or_lactating_mother = models.CharField(
        max_length=100, verbose_name="are you pregnant or lactating mother?", null=True
    )
    registered_for_covid_vaccination = models.CharField(
        max_length=100,
        verbose_name="Have you registered for covid vaccination?",
        null=True,
    )
    registered_phone_number = models.CharField(
        max_length=100, verbose_name="Registered Phone Number", null=True
    )
    take_first_dose = models.CharField(
        max_length=100, verbose_name="Did you take first dose?", null=True
    )
    first_dose_date = models.DateField(null=True, verbose_name="Date of first dose?")
    vaccine_name = models.CharField(
        max_length=100, verbose_name="Which vaccine taken?", null=True
    )
    take_second_dose = models.CharField(
        max_length=100, verbose_name="Did you take second dose?", null=True
    )
    second_dose_date = models.DateField(null=True, verbose_name="Date of second dose?")
    corona_infected = models.CharField(
        max_length=100,
        verbose_name="Have you even been infected with corona?",
        null=True,
    )
    if_corona_infected_days = models.CharField(
        max_length=100,
        verbose_name="If corona infected, how many days it had been since infection?",
        null=True,
    )
    willing_to_vaccinated = models.CharField(
        max_length=100, verbose_name="Are you willing to get vaccinated?", null=True
    )
...
class MemberData(models.Model):
    ...
    household_number = models.CharField(
        max_length=5, null=True, verbose_name="House Number"
    )
    member_uuid = models.CharField(
        max_length=100, verbose_name="Member Uuid", blank=True
    )
    ...
    gender = models.CharField(
        choices=GENDER_CHOICES, max_length=2, blank=True, verbose_name="Gender"
    )
...
class MemberProgramData(models.Model):
    ...
    member = models.ForeignKey(
        MemberData, on_delete=models.CASCADE, verbose_name="Member Id"
    )
    program_uuid = models.CharField(
        max_length=100, verbose_name="Program uuid", blank=True
    )
...
class MemberEncounterData(models.Model):
    ...
    program = models.ForeignKey(
        ...
        verbose_name="Program Enrollment Id",
        null=True,
        blank=True,
    )
    encounter_uuid = models.CharField(
        max_length=100, verbose_name="Encounter uuid", blank=True
    )
    encounter_name = models.CharField(
        max_length=100, verbose_name="Encounter uuid", blank=True
    )
    created_date = models.DateField(null=True, verbose_name="Created date")
    submission_date = models.DateField(null=True, verbose_name="Last Modified date")
...
class MemberDataETL(models.Model):
    ...
    member = models.ForeignKey(
        MemberData, on_delete=models.CASCADE, verbose_name="Slum Id"
    )
    ...
    household_number = models.CharField(max_length=5, null=True, blank=True)
    ...
    mhm_program_enrollment = models.CharField(
        max_length=20, blank=True, verbose_name="Enrollment In MHM"
    )
    available_followup_cnt = models.IntegerField(null=True, blank=True)
    cup_used_in_last_followup = models.CharField(
        max_length=20, blank=True, verbose_name="Cup Used In Last Follow-up"
    )
