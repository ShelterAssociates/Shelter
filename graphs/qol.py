...
waste_coll_to_type = {
    "frequency_of_waste_collection_001": "ulb ghantagadi",
    "frequency_of_waste_collection": "mla sponsored tempo",
    "frequency_of_waste_collection_": "ulb Van",
    "frequency_of_waste_collection__002": "door to door waste collection",
...
def score_calculation(section_key):
    ...
    json_data = json.loads(
        open("graphs/json_reference_file.json").read()
    )  # json reference data from json file
    #    for city_id in all_city_id_list[4:5]:
    slum_data = (
        SlumData.objects.all()
    )  # filter(city_id=city_id)#.values('slum_id','rim_data','city_id')
    for i in slum_data:
        # if i.slum.id == 1117:
        slum__id = i.slum.id
        ...
        for k, v in json_data.items():
            if k == section_key:  # checks sction key like water, toilet etc
                if k in db_data.keys():
                    ...
                    score_list = (
                        []
                    )  # score list for every type of questions (single select , multi select etc)
                    num_dict = {}
                    if type(ques) == dict:
                        ques = [ques]
                    all_ctb_data = []
                                    ...
                                    if k == "Waste":
                                        try:
                                            if k1 in waste_coll_to_type.keys():
                                                ...
                                                score_for = v1["choices"][
                                                    db_data[k][k1].lower()
                                                ]
                                                freq_waste_coll_type[keyy] = score_for
                                        except Exception as e:
                                            logger.error(
                                                "single select waste exception is", e
                                            )
                                        ques_score[k1] = score
                    ...
                    logger.error(
                        "No data for section {} in slum {}".format(
                            section_key, slum__id
                        )
                    )
        for_each_slum = []
        ...
        if section_key == "Waste":
            for_each_slum.append(
                [freq_waste_coll_type, waste_coll_choices, ques_score, slum__id]
            )
            all_slums_list.append(for_each_slum)
...
def waste_final(z):
    ...
    * Coverage of Waste Collection + Community Dump Sites + Dump in Drains"""
    ...
    dumy_di = {}
    waste_coverage_list = [
        "coverage_of_waste_collection_a",
        "coverage_of_waste_collection_a_001",
        "frequency_of_waste_collection__001" "coverage_of_waste_collection_a_002",
        "coverage_of_waste_collection_a_003",
    ]
    ...
    for i in score_data:
        ...
        for i in Count_of_facility:
            ...
            frequency_of_1waste_coll = (
                frequncies_of_waste_coll_facility[i]
                if i in frequncies_of_waste_coll_facility
                else 0
            )
            scr.append(
                count_of_fac_by_HC
...
def toilet_final(z):
    ...
    for i in score_data:
        ...
        id_list.append(id)
        other_data = i[0]
        get_data = DashboardData.objects.filter(slum=i[1]).aggregate(
            Sum("individual_toilet_coverage"), Sum("occupied_household_count")
        )
        toilet_count = (
            get_data["individual_toilet_coverage__sum"]
            if get_data["individual_toilet_coverage__sum"]
            else 0
        )
        total_houses = (
            get_data["occupied_household_count__sum"]
            if get_data["occupied_household_count__sum"]
            else 0
        )
        own_toilet_coverage = round(
            (toilet_count / total_houses) * 100 if total_houses else 0
        )
        men_seats = [
        ...
        for j in other_data:
            ...
            if ctb_data and "is_the_CTB_in_use" in ctb_data[0]:
                ...
                for i in numeric_data.keys():
                    ...
                    wm_fun = 0
                    if i in men_seats:
                        men_fun = int(numeric_data[men_seats[0]]) - int(
                            numeric_data[men_seats[1]]
                        )
                    if i in wm_seats:
                        wm_fun = int(numeric_data[wm_seats[0]]) - int(
                            numeric_data[wm_seats[1]]
                        )
                    if i in mix_seats:
                        mix_fun = int(numeric_data[mix_seats[0]]) - int(
                            numeric_data[mix_seats[1]]
                        )
                    if i in [
                ...
                final_score_of_1_toilet = ctb_in_use * (
                    sum(ctb_data[0].values()) + total_cost + total_wrk_seats
                )
                toilet_scores.append(final_score_of_1_toilet)
            else:
                logger.error("No data for section {} in slum {}".format("Toilet", id))
        final_score = round(
            sum(toilet_scores) / len(toilet_scores) if len(toilet_scores) != 0 else 0, 2
        )
        if own_toilet_coverage <= 25:
            final_score += 10
        elif own_toilet_coverage in range(25, 50):
            final_score += 20
        elif own_toilet_coverage in range(50, 75):
            final_score += 30
        elif own_toilet_coverage >= 75:
            final_score += 40
        else:
            pass
        # print 'final',id, final_score
...
def Rhs_data(slumid):
        ...
        try:
            if i["Type_of_structure_occupancy"] == "Occupied house":
                if "group_el9cl08/Facility_of_solid_waste_collection" in i:
                    waste_data.append(
                        i["group_el9cl08/Facility_of_solid_waste_collection"].lower()
                    )
                else:
    ...
    avg_house_area = (
        0 if (len(gen_data) <= 0) else round((sum(gen_data) / len(gen_data)), 2)
    )
    ...
    rhs_fields = {
        "Waste": [SHC, waste_counter],
        "Water": [SHC, WCT_count],
        "General": avg_house_area,
        "Structure": owner_count,
    }
    return rhs_fields
...
def general_final(z):
    ...
    for i in score_data:
        ...
        household_list = total_household_count.values_list(
            "household_number", flat=True
        )
        followup_data = FollowupData.objects.filter(slum__id=id)
        followup_household_list = followup_data.values_list(
            "household_number", flat=True
        )
        ...
        for i in followup_household_list:
            latest_record = followup_data.filter(household_number=int(i)).latest(
                "submission_date"
            )
            if (
                ...
                if latest_record.followup_data[
                    "group_oi8ts04/Current_place_of_defecation"
                ] in ["Use CTB", "Non-functional, hence CTB"]:
                    ctb_count += 1
                    ...
                    == "Public toilet outside slum"
                ):
                    outside_slum += 1
                else:
                    none_hence_open_def += 1
        status_of_defecation = round(
            ((ctb_count / total_household_rhs_ff) * 2 if total_household_rhs_ff else 0)
            + (
                (own_toilet / total_household_rhs_ff) * 5
                if total_household_rhs_ff
                else 0
            )
            + ((shared / total_household_rhs_ff) * 3 if total_household_rhs_ff else 0)
            + (
                (outside_slum / total_household_rhs_ff) * -1
                if total_household_rhs_ff
                else 0
            )
            + (
                (none_hence_open_def / total_household_rhs_ff) * -2
                if total_household_rhs_ff
                else 0
            ),
            2,
        ...
        for i in other_data:
            if i == "number_of_huts_in_settlement":
                slum_population = 4 * int(
                    other_data[i]
                )  # calculating population of slum
            if (
                i == "approximate_area_of_the_settle"
            ):  # area of slun in hectors(originally obtained is in square meters)
                area_in_hector = (
                    int(other_data[i]) / 10000 if int(other_data[i]) / 10000 != 0 else 1
                )
                ...
                dumy_di[id] = round(final_score, 2)
...
def final_section_score(w):
    ...
    uncommom_keys = all_slum_ids - set(
        d0.keys()
        and d1.keys()
        and d2.keys()
        and d3.keys()
...
def percentile_function():
    ...
    for city_id in all_city_id_list:
        from_db = QOLScoreData.objects.filter(
            city_id=city_id
        ).values()  # for percentile calculations need all data
        for i in from_db:
    ...
    try:
        total_score_pert = {
            i: stats.percentileofscore(total_score_dict.values(), total_score_dict[i])
            for i in total_score_dict.keys()
        }
        toilet_pert = {
            i: stats.percentileofscore(toilet_dict.values(), toilet_dict[i])
            for i in toilet_dict.keys()
        }
        road_pert = {
            i: stats.percentileofscore(road_dict.values(), road_dict[i])
            for i in road_dict.keys()
        }
        water_pert = {
            i: stats.percentileofscore(water_dict.values(), water_dict[i])
            for i in water_dict.keys()
        }
        waste_pert = {
            i: stats.percentileofscore(waste_dict.values(), waste_dict[i])
            for i in waste_dict.keys()
        }
        drainage_pert = {
            i: stats.percentileofscore(drainage_dict.values(), drainage_dict[i])
            for i in drainage_dict.keys()
        }
        gutter_pert = {
            i: stats.percentileofscore(gutter_dict.values(), gutter_dict[i])
            for i in gutter_dict.keys()
        }
        general_pert = {
            i: stats.percentileofscore(general_dict.values(), general_dict[i])
            for i in general_dict.keys()
        }
        stru_n_ocup_pert = {
            i: stats.percentileofscore(stru_n_ocup_dict.values(), stru_n_ocup_dict[i])
            for i in stru_n_ocup_dict.keys()
        }

        for i in slum_id_list:
            if (
                i["slumid"] in road_pert.keys()
                and waste_pert.keys()
                and water_pert.keys()
                and gutter_pert.keys()
                and general_pert.keys()
                and stru_n_ocup_pert.keys()
                and toilet_pert.keys()
                and total_score_pert.keys()
                and drainage_pert.keys()
            ):
                slum_percentile_sectionwise[i["slumid"]] = {
                    "Waste": waste_pert[i["slumid"]],
                    "Road": road_pert[i["slumid"]],
                    "Water": water_pert[i["slumid"]],
                    "Gutter": gutter_pert[i["slumid"]],
                    "General": general_pert[i["slumid"]],
                    "Str_n_occup": stru_n_ocup_pert[i["slumid"]],
                    "Drainage": drainage_pert[i["slumid"]],
                    "Total_score": total_score_pert[i["slumid"]],
                    "Toilet": toilet_pert[i["slumid"]],
                }

            for k, v in slum_percentile_sectionwise.items():
                if i["slumid"] == k:
                    trial = QOLScoreData.objects.filter(id=i["id"], slum_id=k).update(
                        totalscore_percentile=v["Total_score"],
                        toilet_percentile=v["Toilet"],
                        str_n_ocup_percentile=v["Str_n_occup"],
                        road_percentile=v["Road"],
                        drainage_percentile=v["Drainage"],
                        general_percentile=v["General"],
                        gutter_percentile=v["Gutter"],
                        waste_percentile=v["Waste"],
                        water_percentile=v["Water"],
                    )
                else:
                    pass
    except Exception as e:
        logger.error("Exception in percentile function", e)
...
def QOL_save_data(request):
    ...
    for i in result.items():
        ...
        if i[0] in slum_city_list.keys():
            try:
                to_save = QOLScoreData.objects.update_or_create(
                    slum_id=i[0],
                    defaults={
                        "water": data["Water"],
                        "waste": data["Waste"],
                        "road": data["Road"],
                        "str_n_occup": data["Str_n_occup"],
                        "city_id": slum_city_list[i[0]],
...
def single_select(db_data, k1, v1):
    ...
    if ans.lower() in v1["choices"]:
        ...
        logger.error("exception in single select ", ans, k1)
        score = 0
    return score
...
def multiselect(db_data, k1, v1):
    ...
    try:
        ...
        logger.error("exception in multi select ", ans, k1)
        score = {k1: 0}
    return score
        ...
        if "Structure" in data.keys():
            ...
            try:
                if len(owner_status) == 0:
                    ...
                    score = (
                        1
                        if (owner_status["Own house"] >= owner_status["Tenant"])
                        else -1
                    )
                dumy_di[id] = score
            except Exception as e:
                logger.error("exception in structure n occup", e, owner_status)
    return dumy_di
...
def water_final(z):
    ...
    * Weight of that particular facility) for all facilities individually
    ...
    dumy_di = {}
    weight_of_facility = {
        "Individual connection": 5,
        "Shared connection": 4,
        "Hand Pump": 2,
        "Well": 2,
        "Water Tanker": 2,
        "Water standpost": 2,
        "From other settlements": -2,
        "No Service": -5,
        "None": 0,
    }
        ...
        if "Water" in water_info.keys():
            ...
            for i in water_conn_facility_count:
                if i in weight_of_facility.keys():
                    wwf_count = (
                        water_conn_facility_count[i] / total_household
                    ) * weight_of_facility[
                        i
                    ]  # wwf=working water facility count
