...
def fetch_labels_codes(rhs_data, form_code):
    name_label_data = []
    url_rhs_form = (
        str(settings.KOBOCAT_FORM_URL) + "forms/" + str(form_code) + "/form.json"
    )
    ...
    for i in formdict_RHS_form["children"]:
        temp_data = trav(
            i
        )  # trav() function traverses the dictionary to find last hanging child
        name_label_data.extend(temp_data)

    name_label_data_dict = {
        obj_name_label_data["name"]: {
            child["name"]: child["label"] for child in obj_name_label_data["children"]
        }
        for obj_name_label_data in name_label_data
    }
...
def fetch_data(form_code, latest_date):
    ...
    if latest_date != "":
        rhs_url += (
            '&query={"end":{"$gt":"'
            + urllib2.quote(str(timezone.localtime(latest_date)))
            + '"}}'
        )
...
def syn_rim_data(city_id):
    ...
    for city in cities:
        survey = Survey.objects.filter(
            city__id=int(city.id), description__contains="RIM"
        )
        if survey:
            kobo_survey = survey[0].kobotool_survey_id

            latest_rim = (
                SlumData.objects.filter(city=city).order_by("-submission_date").first()
            )
            ...
            for record in submission:
                if "group_zl6oo94/group_uj8eg07/slum_name" in record:
                    ...
                    try:
                        ...
                        slum_data, created = SlumData.objects.update_or_create(
                            slum=slum, defaults=data
                        )
                        ...
                        logger.error(
                            f"Slum with shelter_slum_code {key} does not exist. Error: {e}"
                        )
...
def syn_rhs_followup_data(city_id, ff_flag=False, latest_flag=True, start_date=None):
    ...
    for city in cities:
        ...
        survey_forms_rhs = Survey.objects.filter(
            city__id=int(city.id), description__contains="RHS"
        )
        for i in survey_forms_rhs:
            ...
            latest_rhs = HouseholdData.objects.filter(city__id=city.id).order_by(
                "-submission_date"
            )
            if len(latest_rhs) != 0 and latest_flag:
                latest_rhs_date = latest_rhs[0].submission_date

            latest_followup = FollowupData.objects.filter(city__id=city.id).order_by(
                "-submission_date"
            )
            ...
            latest_date = (
                ...
                if latest_followup_date > latest_rhs_date
                else latest_rhs_date
            )
            ...
            if True:  # city.id == 1: #3,4 done
                ...
                data_with_lables = [
                    x for x in data_with_lables if "slum_name" in x.keys()
                ]
                data_with_lables = [
                    x
                    for x in data_with_lables
                    if "Type_of_structure_occupancy" in x.keys()
                ]
                data_with_lables = sorted(
                    data_with_lables, key=lambda x: x["slum_name"]
                )

                for key, list_records in groupby(
                    data_with_lables, lambda x: x["slum_name"]
                ):
                    try:
                        slum = Slum.objects.get(shelter_slum_code=key)
                    except:
                    ...
                    for record in list_records:
                        if "group_oi8ts04/C1" in record.keys():
                            record.update(
                                {
                                    "group_oi8ts04/Current_place_of_defecation": record[
                                        "group_oi8ts04/C1"
                                    ]
                                }
                            )
                        ...
                        if record["Type_of_structure_occupancy"] in [
                            "Unoccupied house",
                            "Locked house",
                        ]:
                            if (
                                record["Type_of_structure_occupancy"]
                                == "Unoccupied house"
                            ):
                ...
                for key, list_records in groupby(
                    ...
                    for index, record in enumerate(list_records):
                        ...
                        if record["Type_of_structure_occupancy"] == "Occupied house":
                            if (
                                "group_og5bx85/Type_of_survey" in record
                                and record["group_og5bx85/Type_of_survey"] == "RHS"
                            ):
                                f_data = {}
                                r_data = {}
                                ...
                                try:
                                    try:
                                        hh_data = HouseholdData.objects.filter(
                                            kobo_id=record["_id"]
                                        )
                                        if (
                                            hh_data.count() > 0
                                    ...
                                    followup_Data = FollowupData.objects.filter(
                                        household_number=str(
                                            int(record["Household_number"])
                                        ),
                                        slum=slum,
                ...
                for key, list_records in groupby(
                    ...
                    for record in list_records:
                        ...
                        if record["Type_of_structure_occupancy"] == "Occupied house":
                            if (
                                "group_og5bx85/Type_of_survey" in record
                                and record["group_og5bx85/Type_of_survey"]
                                == "Follow-up survey"
                            ):
                                ...
                                try:
                                    ...
                                    try:
                                        followup = FollowupData.objects.filter(
                                            kobo_id=f_data["_id"]
                                        )
                                        if followup.count() > 0:
                                            followup.update(
...
def sync_ff_data(city_id, latest_date=""):
    cities = City.objects.filter(id__in=[city_id])
    for city in cities:
        survey_forms_ff = Survey.objects.filter(
            city__id=int(city.id), description__contains="FF"
        )
        for i in survey_forms_ff:
            ...
            ff_data_with_labels = [
                x for x in ff_data_with_labels if "group_vq77l17/slum_name" in x.keys()
            ]

            ff_data_with_labels = sorted(
                ff_data_with_labels, key=lambda x: x["group_vq77l17/slum_name"]
            )  # group_vq77l17/slum_name
            for key, list_records in groupby(
                ff_data_with_labels, lambda x: x["group_vq77l17/slum_name"]
            ):
                slum = None
                try:
                    slum = Slum.objects.get(shelter_slum_code=key)
                ...
                if slum:
                    ...
                    for record in list_records:
                        try:
                            if (
                                "group_im2th52/Approximate_monthly_family_income_in_Rs"
                                in record
                            ):
                                record[
                                    "group_im2th52/Approximate_monthly_family_income_in_Rs"
                            ...
                            if "group_ne3ao98/Cost_of_upgradation_in_Rs" in record:
                                record["group_ne3ao98/Cost_of_upgradation_in_Rs"] = int(
                                    "0"
                                    + "".join(
                                        re.findall(
                                            "[\d+]",
                            ...
                            hh_data = HouseholdData.objects.filter(
                                ff_kobo_id=record["_id"]
                            )
                            if (
                                hh_data.count() > 0
                                and hh_data.filter(
                                    household_number=str(
                                        int(record["group_vq77l17/Household_number"])
                                    ),
                                    slum=slum,
                                ).count()
                                == 0
                            ...
                            temp = HouseholdData.objects.filter(
                                household_number=str(
                                    int(record["group_vq77l17/Household_number"])
                                ),
                                slum=slum,
