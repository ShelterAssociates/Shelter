...
class avni_sync:
    ...
    def process_record(self, r, record_num, total_records):
        ...
        try:
            ...
            put_response = self.make_request(
                "PUT", self.base_url + "api/subject/" + subject_id, data=payload
            )
            ...
            logger.error(subject_id, "Error:", e)
    ...
    def subject_data_update_parallel(self):
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            futures = [
                executor.submit(self.process_record, r, i + 1, total_records)
            ...
            for future in as_completed(futures):
                ...
                if status:
                    conn.execute(
                        f'UPDATE "{table_name}" SET status = ? WHERE uuid = ?',
                        (status, uuid),
                    )
                    conn.commit()
                if res["not_found"]:
        ...
        for i, r in df.iterrows():
            try:
                ...
                if Request.status_code == 200:
                    ...
                    cur.execute(
                        f"Update \"{table_name}\" set status = 1 where uuid = '{subject_id}'"
                    )
                    ...
                    headers = {
                        "auth-token": self.get_cognito_token(),
                        "Content-Type": "application/json",
                    }
                    ...
                    if response.status_code != 200:
                        # Put Failed status = 4
                        cur.execute(
                            f"Update \"{table_name}\" set status = 4 where uuid = '{subject_id}'"
                        )
                        final_list.append(subject_id)
                    else:
                        # print("Record", subject_id, "Updated Successfully")
                        cur.execute(
                            f"Update \"{table_name}\" set status = 3 where uuid = '{subject_id}'"
                        )
                        count += 1
                else:
                    # Get failed
                    cur.execute(
                        f"Update \"{table_name}\" set status =2  where uuid = '{subject_id}'"
                    )
                conn.commit()
            ...
            try:
                ...
                if Request.status_code == 200:
                    ...
                    if data["observations"].get("Activity conducted for project ?"):
                        ...
                        data["observations"]["Activity conducted for project ?"] = r[
                            "Activity conducted for project ?"
                        ]
