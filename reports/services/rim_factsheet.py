...
def map_rim_data(raw_data: dict) -> dict:
    ...
    for section_name, fields in section_definitions.items():
        ...
        if section_name == "Toilet":
            ...
            section_fields["Number of Seats (male / female / mixed)"] = (
        ...
        if section_name == "Water":
            ...
            section_fields["Total Number of Stand-Posts (in use / NOT in use)"] = (
...
def get_rim_factsheet_detail(slum_code):
    ...
    # ---- Fetch slum
    slum = Slum.objects.only("id", "shelter_slum_code").filter(id=slum_code).first()
    if not slum:
        return result
...
def _fetch_signed_url(field_name, raw_url, token):
    try:
        resp = requests.get(
            AVNI_SIGNED_URL_API + raw_url, headers={"AUTH-TOKEN": token}, timeout=10
        )
...
def resolve_rim_images(rim_record):
    ...
    with ThreadPoolExecutor(max_workers=6) as executor:
        for key, url in executor.map(
            lambda item: _fetch_signed_url(item[0], item[1], token), avni_images.items()
        ):
            rim_record[key] = url
