...
def monthly_report_details(request, report_id):
    ...
    report = (
        SponsorProjectMonthlyReportDetails.objects.select_related(
            "project_report",
    ...
    data = {
        "project": {
            ...
            "sponsor_name": (
                project.sponsor_project.sponsor.organization_name
                if project.sponsor_project.sponsor
                else None
            ),
            "start_month": (
                project.start_month.strftime("%b %Y") if project.start_month else None
            ),
            "end_month": (
                project.end_month.strftime("%b %Y") if project.end_month else None
            ),
        },
        "month": report.month.strftime("%b %Y") if report.month else None,
        "locations": [
            format_location(loc.name) for loc in project.project_locations.all()
        ],
    ...
    for row in report.work_progress.all():
        ...
        work_progress[loc].append({"parameter": row.parameter.name, "value": row.value})

    data["work_progress"] = [
        {"location": loc, "parameters": params} for loc, params in work_progress.items()
    ]
    ...
    data["beneficiary_values"] = [
        {
            ...
            "value": row.value,
            "icon": (
                request.build_absolute_uri(row.indicator.icon.url)
                if row.indicator.icon
                else None
            ),
        }
    ...
    target_map = {d.deliverable_id: d for d in project.project_deliverables.all()}
        ...
        data["deliverables"].append(
            {
                ...
                "unit": unit,
                "icon": (
                    request.build_absolute_uri(r.deliverable.icon.url)
                    if r.deliverable.icon
                    else None
                ),
                "percent": percent,
            }
...
def create_monthly_reports(project_report):
    ...
    while current <= end:
        ...
        report, created = SponsorProjectMonthlyReportDetails.objects.get_or_create(
            project_report=project_report, month=current
        )
...
def initialize_beneficiary_values(monthly_report):
    ...
    for indicator in indicators:
        ...
        rows.append(
            SponsorProjectMonthlyBeneficiaryValue(
                monthly_report=monthly_report, indicator=indicator, value=0
            )
        )

    SponsorProjectMonthlyBeneficiaryValue.objects.bulk_create(
...
def initialize_deliverables(monthly_report):
    ...
    for d in deliverables:
        ...
        rows.append(
            SponsorProjectMonthlyDeliverableAchievement(
                monthly_report=monthly_report, deliverable=d.deliverable, value=0
            )
        )

    SponsorProjectMonthlyDeliverableAchievement.objects.bulk_create(
        rows, ignore_conflicts=True
    )
