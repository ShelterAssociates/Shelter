...
urlpatterns = [
    url(
        r"^api/rim_factsheet_generation/(?P<slum_id>[0-9]+)/$",
        rim_factsheet_pdf_generation,
        name="rim_factsheet_pdf_generating",
    ),
    url(
        r"^api/rim_factsheet_pdf_fetch/(?P<slum_id>[0-9]+)/$",
    ...
    url(
        r"^fetch-donor-report/(?P<report_id>[0-9]+)/$",
        monthly_donor_report_pdf_fetch,
        name="fetch_donor_report",
    ),
    url(r"^api/donor-projects/$", donor_projects, name="donor_projects_api"),
    url(
        r"^api/project-months/(?P<project_id>[0-9]+)/$",
        project_months,
        name="monthly_report_api",
    ),
]
