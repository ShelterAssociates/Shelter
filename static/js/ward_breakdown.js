...
// ── Line-feature ward matching (centroid + point-in-polygon) ───────────────
// Line components (roads, drainage, etc.) are deliberately excluded from the
// server-side housenumber-based `_wb.componentWards` assignment (see
// `_fetch_countable_components` in component/services/helper.py) — "which
// ward contains this line" isn't answerable by housenumber matching. Their
// per-ward LENGTH is computed correctly server-side via `_wb.wardRoadLengths`
// (PostGIS clip + sum) and must stay untouched by this. For DRAWING the line
// on the map when a ward is active, fall back to a client-side centroid +
// point-in-polygon test against the ward boundary shapes — restored here
// (previously lived in master_map_component.js, removed in commit
// 056e23d6 when ward assignment moved server-side for non-line components).
...
// Rather than silently dropping those, fall back to whichever ward's
// boundary the point is closest to.
...
function _showComponentForActiveWard(componentName) {
    ...
    if (type === "C") {
        ...
        // Line-type components (roads, drainage, etc.) are never populated
        // in the server-side housenumber assignment (`_wb.componentWards`)
        // — see `_fetch_countable_components` in
        // component/services/helper.py. Detect them the same way the
        // length label already does (`_wbComputeCountsForWard`): if this
        // ward has a pre-computed clipped length for this component name,
        // it's a line component. Match it to the active ward via centroid +
        // point-in-polygon against the ward boundary shapes instead.
        var wardRoadLengthsForActive = _wb.wardRoadLengths[String(_wb.activeWardId)] || null;
        var isLineComponent = !!(wardRoadLengthsForActive &&
            Object.prototype.hasOwnProperty.call(wardRoadLengthsForActive, componentName));
        ...
        if (isLineComponent) {
            $.each(itemData.child || [], function (_, childItem) {
                ...
                if (_wardContainsOrNearest(_wb.wardShapes, _wb.activeWardId, centroid.lat, centroid.lng)) {
                    filteredShapes.push(childItem.shape);
                }
            });
        } else {
