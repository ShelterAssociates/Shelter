...
// Function for handling the ajax call for the Sponsor project selection after the sponsor selection.
function sponsorprojectList() {
  // $("#sponsor_project").empty();
  var id = $("#sponsor_name option:selected").val();
  $.ajax({
    url: "/mastersheet/sponsorprojectList/",
    data: { id: id },
    type: "POST",
    contenttype: "json",
    success: function (json) {
      ...
      $("#sponsor_project").append(str);
      for (i = 0; i < json.nameArray.length; i++) {
        var str = '<option value="';
        str =
          str + json.idArray[i] + '"' + ">" + json.nameArray[i] + "</option>";
        $("#sponsor_project").append(str);
      }
    },
  });
...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (data, type, row, meta) {
      ...
      for (i = 0; i < tmp_RHS.length; i++) {
        columns_defs["data"][tmp_RHS[i]]["render"] = function (
          data,
          type,
          row,
          meta
      ...
      for (i = 0; i < tmp_FF.length; i++) {
        columns_defs["data"][tmp_FF[i]]["render"] = function (
          data,
          type,
          row,
          meta
      ...
      for (i = 0; i < tmp_download_TF.length; i++) {
        //Toilet photo
        columns_defs["data"][start_of_FF + 6]["render"] = function (
          data,
          type,
          row,
          meta
        ...
        //Family photo
        columns_defs["data"][start_of_FF + 5]["render"] = function (
          data,
          type,
          row,
          meta
      ...
      for (i = 0; i < tmp_DR.length; i++) {
        columns_defs["data"][tmp_DR[i]]["render"] = function (
          data,
          type,
          row,
          meta
          ...
          if (typeof data != "undefined") {
            url_daily_reporting =
              String("/accounts/mastersheet/communitymobilization/") +
              row[columns_defs["data"][meta.col]["title"] + "_id"] +
              String("/");
            if (data.length > 11) {
              url_daily_reporting =
                String(
                  "/accounts/mastersheet/communitymobilizationactivityattendance/"
                ) +
                row[columns_defs["data"][meta.col]["title"] + "_id"] +
                String("/");
            }
      ...
      for (i = 0; i < tmp_ACC.length; i++) {
        columns_defs["data"][tmp_ACC[i]]["render"] = function (
          data,
          type,
          row,
          meta
          ...
          if (typeof data != "undefined") {
            url_accounts =
              String("/accounts/mastersheet/invoice/") +
              row[columns_defs["data"][meta.col]["title"] + "_id"] +
              String("/");
            if (type === "display") {
              data =
      ...
      for (i = 0; i < tmp_SBM.length; i++) {
        columns_defs["data"][tmp_SBM[i]]["render"] = function (
          data,
          type,
          row,
          meta
          ...
          if (typeof data != "undefined") {
            url_SBM =
              String("/accounts/mastersheet/sbmupload/") +
              row["sbm_id_" + String(row.Household_number)] +
              String("/");
            if (type === "display") {
              data =
                '<a href = "#" onclick="window.open(\'' +
        ...
        columns_defs["data"][columns_defs["buttons"]["RHS"][0] - 1]["render"] =
          function (data, type, row, meta) {
            if (typeof data != "undefined") {
              if (typeof row["Household_id"] != "undefined") {
                url_SBM =
                  String("/accounts/graphs/householddata/") +
                  row["Household_id"] +
                  String("/");
                if (type === "display") {
                  data =
              ...
              if (row["no_rhs_flag"] != "") {
                data =
                  "<p class = 'highlight_p' style = 'background-color : " +
                  row["no_rhs_flag"] +
                  ";'>" +
                  data +
                  "</p>";
      ...
      for (i = 0; i < tmp_TC.length; i++) {
        if (columns_defs["data"][tmp_TC[i]]["data"] != "Funder") {
          columns_defs["data"][tmp_TC[i]]["render"] = function (
            data,
            type,
            row,
            meta
            ...
            if (typeof data != "undefined") {
              url_TC =
                String("/accounts/mastersheet/toiletconstruction/") +
                row["tc_id_" + String(row.Household_number)] +
                String("/");
              if (type === "display") {
                data =
                  '<a href = "#" onclick="window.open(\'' +
        ...
        if (
          columns_defs["data"][tmp_TC[i]]["data"] ==
        ...
        btn_default = [
          {
            ...
            exportOptions: {
              rows: ".highlight_p",
              columns: [...Array(length_table).keys()].slice(
                columns_defs["buttons"]["RHS"][0] - 2,
                length_table
              ),
            },
          },
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            jQuery.each(response, function (index, value) {
              ...
              try {
                if (index.indexOf("No Households") != -1) {
                  total_not_available = total_not_available + value[0] + ", ";
                }
              } catch (error) {
            ...
            success_log.innerHTML =
              "<p>Number of records in the uploaded sheet: " +
              response.total_records +
              "<br>Total records updated: " +
              total_updates +
              "<br>Number of new records added: " +
              total_new +
              "<br>No Household Available: " +
              total_not_available +
              "</p>";
  ...
  $("#downloadGIS").on("click", function () {
    if (
      $("#gisdata_slumname").val() == "" &&
      $("#gisdata_cityname").val() == ""
    ) {
  ...
  function load_data_datatable() {
    if (table != null) {
          ...
          dataSrc: function (data) {
            for (i = 0; i < data.length; i++) {
              if (data[i] != null) {
                ...
                data[i]["p3_missing"] = "";
                if (
                  !data[i]["agreement_date_str"] &&
                  (data[i]["phase_one_material_date_str"] ||
                    data[i]["phase_two_material_date_str"] ||
                    data[i]["phase_three_material_date_str"] ||
                    data[i]["completion_date_str"])
                ) {
      ...
      $(
        "#p1, #p2, #p3, #cd, #ad, #md, #wo, #no_rhs, #material_shift, #incorrect_COD"
      ).click(function () {
        var selected = [];
        boxes = [];
        $.each($("input[name='checkbox_filter']:checked"), function (k, v) {
          boxes.push($(v).attr("value"));
        });
        table.draw();
      });
      $.fn.dataTable.ext.search.push(function (settings, data, dataIndex) {
        if (
          boxes.indexOf(data[9]) > -1 ||
          ($("#incorrect_COD:checked").length == 1 &&
            data[13] == "incorrect_cpod") ||
          ($("#material_shift:checked").length == 1 && data[12] == "#f9cb9f") ||
          ($("#no_rhs:checked").length == 1 && data[11].length > 1) ||
          ($("#wo:checked").length == 1 && data[10] == "Written-off") ||
          ($("#md:checked").length == 1 &&
            (data[0] != "" ||
              data[1] != "" ||
              data[2] != "" ||
      ...
      $("#delete_selected").click(function () {
        ...
        if (count == 0) {
          alert(
            "You have not selected any record. Please select a record to delete."
          );
        } else {
          var records = [];
          for (i = 0; i < count; i++) {
            records[i] = table.rows(".selected").data()[i]["Household_number"];
          }
          if (records.length == 1) {
            var result = confirm(
          ...
          if (result) {
            ...
            $.ajax({
              ...
              data: JSON.stringify({
                records: records,
                slum: document.forms[0].slumname.value,
              }),
              contentType: "json",
      ...
      $.each(columns_defs["buttons"], function (key, val) {
        ...
        html_table
          .find(
            "thead>tr>th:eq(" + (val[0] - number_of_invisible_columns) + ")"
          )
          .addClass("trFirst");
        $.each(val.slice(1, val.length - 1), function (k, v) {
          html_table
            .find("thead>tr>th:eq(" + (v - number_of_invisible_columns) + ")")
            .addClass("trMiddle");
        });
        $.each(val, function (k, v) {
          ...
          if (String(key) === "RHS") {
            html_table
              .find("thead>tr>th:eq(" + va + ")")
              .css("background-color", "#d6d0dd"); //
    ...
    if (report_short_datatable != null) {
      ...
      report_short_datatable = $("#example").DataTable({
        ...
        columns: [
          ...
          {
            data: "name_head_of_he_household",
            title: "Name of the head of the household",
          },
          {
            data: "current_place_of_defication",
            title: "Current place of defecation",
          },
          {
            data: "phase_one_material_date",
            title: "Date of Phase One Material",
          },
          {
            data: "phase_two_material_date",
            title: "Date of Phase Two Material",
          },
          {
            data: "phase_three_material_date",
            title: "Date of Phase Three Material",
          },
          ...
          {
            data: "total_moilization_acticity",
            title: "No. of mobilisation activities attended by family",
          },
          ...
          { data: "factsheetAssignStatus", title: "Factsheet Assign Status" },
        ],
      });
      $("div.dt-buttons>button").addClass("pull-left");
      add_search_box();
      $(report_short_datatable.table().container()).on(
        "keyup ",
        "tfoot tr th input",
        function (index, element) {
          report_short_datatable
            .column($(this).attr("dt_index"))
            .search(String(this.value))
            .draw();
      ...
      select_rows();
      $("#add_table_btn").show();
      // Event handler for adding sponsor
      // Process selected rows
      var records = [];
      $("#add_sponsor").on("click", function () {
        var selectedRows = report_short_datatable.rows(".selected").data();
        if (selectedRows.length === 0) {
          alert("Please select a record to add a sponsor.");
          return;
        }
        records = [];
        var no_ff_assign = null;
        for (var i = 0; i < selectedRows.length; i++) {
          var rowData = selectedRows[i];
          if (rowData["factsheetAssignStatus"] !== "Ready To Assign") {
            no_ff_assign = rowData["household_number"];
            continue;
          }
          records.push(rowData["household_number"]);
        }
        ...
        // Show sponsor form modal and handle form submission
        $("#SponsorModel").modal("show");
      });

      // Event handler for sponsor form submission
      $("#addSponsor").on("click", function () {
        // Validate form inputs (sponsor_name, sponsor_project, sponsor_quarter)
        var sponsorId = $("#sponsor_name").val();
        var projectId = $("#sponsor_project").val();
        var quarterId = $("#sponsor_quarter").val();
        ...
        if (
          ...
          alert(
            "Please select valid options for Sponsor Name, Project Name, and Quarter."
          );
          return;
        }

        // Proceed with AJAX request to add sponsor
        var requestData = {
          records: records,
        ...
        $.ajax({
          type: "post",
          url: "/mastersheet/addSponsor/",
          data: JSON.stringify(requestData),
          contentType: "json",
          success: function (response) {
            $(".overlay").hide();
            if (response.response != "") {
              if (window.confirm("Sponsor Added !! Click Ok to see ...")) {
                window.open(response.response, "_blank");
              }
              // Reset form and clear selected rows
              $("#SponsorModel_selection")[0].reset();
              $("#SponsorModel").modal("hide");
              $("#btnFetchShort").click(); // Reload DataTable
            }
          },
          error: function (resp) {
            $(".overlay").hide();
      ...
      // Event handler for sponsor name change (optional)
      $("#sponsor_name").on("change", function () {
        sponsorprojectList(); // Function to populate sponsor project choices
      });
    }
  }
