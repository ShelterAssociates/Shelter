...
function Point_string(){	
    ...
    for(var i=0; i<PointArray.length;i++){
       	lng = PointArray[i].lng();
       	lat = PointArray[i].lat();
...
function Point_string_update(){    
    ...
    for(var i=0; i<curLatLng.length;i++)
    {
       lng = curLatLng[i].lng();
       lat = curLatLng[i].lat();
       string_append+=lng+" "+lat +",";
...
$(document).ready(function(){
	...
	{
	  	django.jQuery('#id_shape').val(Point_string_update());
        django.jQuery('#id_shape').text(Point_string_update());
	}      		
