...
def send_email(to_emails, subject, template_name=None, context=None, plain_message="", thread_message_id=None, cc=None, bcc=None):
    ...
    cc = cc or []
    bcc = bcc or []
    logger.info("Email render starting: subject=%s to=%s template=%s cc=%s bcc=%s", subject, to_emails, template_name, cc, bcc)
    html_content = render_to_string(template_name, context) if template_name else plain_message
    logger.info("Email render finished: subject=%s to=%s template=%s html_length=%s", subject, to_emails, template_name, len(html_content or ""))
    ...
    logger.info("Email send starting: subject=%s to=%s cc=%s bcc=%s message_id=%s", subject, to_emails, cc, bcc, message_id)
