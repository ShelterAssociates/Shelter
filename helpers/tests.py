...
class PhotoUploadHelpersTests(SimpleTestCase):
    ...
    def test_get_slum_hierarchy_uses_all_master_levels(self):
        ...
        electoral_ward = type(
            "ElectoralWard",
            (),
            {"name": "Electoral 5", "administrative_ward": admin_ward},
        )()
        slum = type(
            "Slum", (), {"name": "Janta Nagar", "electoral_ward": electoral_ward}
        )()
    ...
    def test_build_drive_path_supports_normal_city_level_and_custom_uploads(self):
        ...
        electoral_ward = type(
            "ElectoralWard", (), {"name": "E5", "administrative_ward": admin_ward}
        )()
        slum = type(
            "Slum", (), {"name": "GaneshNagar", "electoral_ward": electoral_ward}
        )()
        ...
        class PhotoType:
            ...
            def path_nodes(self):
                return [
                    type("Node", (), {"name": "Activity"})(),
                    type("Node", (), {"name": "Festival"})(),
                ]

        photo_type = PhotoType()

        normal = build_drive_path(
            "2026-05-28", project_type="PHOT", photo_type_item=photo_type, slum=slum
        )
        self.assertEqual(
            normal["drive_path_parts"],
            [
                "PHOT",
                "Pune",
                "GaneshNagar_A12_E5",
        ...
        city_level = build_drive_path(
            "2026-05-28",
            project_type="MHM",
            photo_type_item=photo_type,
            city=city,
            is_city_level=True,
        )
        self.assertEqual(
            city_level["drive_path_parts"],
            ["MHM", "Pune", "Activity", "Festival", "2026-05-28"],
        )

        custom = build_drive_path(
            "2026-05-28",
            project_type="Other",
            project_type_other="Community",
            is_other_upload=True,
            custom_folder_name="FloodReliefPhotos",
        )
        self.assertEqual(
            custom["drive_path_parts"],
            ["Other", "Community", "FloodReliefPhotos", "2026-05-28"],
        )
