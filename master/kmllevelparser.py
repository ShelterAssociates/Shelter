        ...
        if hasattr(placemark, POLYGON):
            ...
            if hasattr(placemark, "MultiGeometry"):
                for coord in placemark["MultiGeometry"][POLYGON]:
                    geometry_data.append(
                        str(coord.outerBoundaryIs.LinearRing.coordinates)
                    )
    ...
    def parse_kml(self):
        ...
        for folder in self.folders:
            for pm in folder.Placemark:
                ...
                if getattr(pm, "ExtendedData", None) and self.action_title in [
                    "ElectoralWard",
                    "Slum",
                ]:
                        ...
                        for simple_data in schema_data.SimpleData:
                            ...
                            if name_attr == "admin":
                                ...
                                if admin_name:
                                    ...
                                    parent_filter["defaults"][
                                        base_filter_data["ElectoralWard"][1]
                                    ] = electoral_obj
                ...
                for count_c, coordinate in enumerate(coordinates):
                    if self.action_title == "ElectoralWard":
                        name += (
                            " "
                            + base_filter_data[self.action_title][1]
                            .replace("_", " ")
                            .title()
                        )
                    ...
                    obj, created = self.parent_model.objects.update_or_create(
                        **parent_filter
                    )
                    if created == True:
                        cnt["created"] += 1
    ...
    def linking(self):
        try:
            for parent_level, child_level in parse_link.items():
                ...
                parent_level_objects = parent_level_model.objects.filter(
                    **parent_level_filter
                )

                for parent_level_object in parent_level_objects:
                    child_level_objects = child_level_model.objects.filter(
                        **update_level
                    )
                    for child_level_object in child_level_objects:
                        if parent_level_object.shape.intersects(
                            child_level_object.shape
                        ):
                            filter_data_child = {}
                            filter_data_child[base_filter_data[parent_level][1]] = (
                                parent_level_object
                            )
                            child_level_model.objects.filter(
                                id=child_level_object.id
                            ).update(**filter_data_child)
