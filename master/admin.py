...
from master.models import (
    ...
    Rapid_Slum_Appraisal,
    Survey,
    drainage,
    SlumTransformationPhase,
    SlumTransformationImage,
)
...
class UploadKMLBase(admin.ModelAdmin):
    ...
    def process_action(self, request, city_id, action_title):
        ...
        if request.method == "POST":
            ...
            try:
                kml_level_parser = KMLLevelParser(
                    docFile, city_id, chk_delete, action_title
                )
                cnt = kml_level_parser.parse_kml()

                if cnt["created"] > 0:
                    response["message"] = (
                        "Created " + str(cnt["created"]) + " " + action_title
                    )
                if cnt["updated"] > 0:
                    response["message"] += (
...
class BaseModelAdmin(admin.ModelAdmin):
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        ...
        if db_field.name == "electoral_ward":
            kwargs["queryset"] = ElectoralWard.objects.filter(
                administrative_ward__city__name__city_name__in=group_perm
            )
        if db_field.name == "administrative_ward":
            kwargs["queryset"] = AdministrativeWard.objects.filter(
                city__name__city_name__in=group_perm
            )
...
class ElectoralWardFormAdmin(BaseModelAdmin):
    ...
    list_display = ("name", "ward_no", "ward_code", "administrative_ward", "city_name")
    search_fields = ("name",)
