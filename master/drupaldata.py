...
def get_electoral_wards(drupal_admin_id, local_ward_id):
    ...
    for n in json.loads(response.data)["nodes"]:
        ...
        key = "Polygon"
        pnt = GEOSGeometry(
            '{ "type": "' + key + '" , "coordinates": ' + str(lst_coordinates) + "  }"
        )

        admin_data = AdministrativeWard.objects.get(id=local_ward_id)
        obj, created = ElectoralWard.objects.update_or_create(
            name=n["node"]["name"],
            administrative_ward=admin_data,
            shape=pnt,
            extra_info=n["node"]["description"],
            defaults={"name": n["node"]["name"]},
        )
        print(created)
