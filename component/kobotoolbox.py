        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                survey = Survey.objects.filter(
                    city__id=city_id, survey_type=survey_type
                )
            except:
                survey = None
            kwargs["kobo_survey"] = ""
...
def get_household_analysis_data(city, slum_code, question_fields, kobo_survey=""):
    ...
    household_rows = list(
        HouseholdData.objects.filter(slum=slum).exclude(id__in=hide_houdehold_data)
    )
    records = [row.rhs_data for row in household_rows if row.rhs_data is not None]
    _log_analysis_timing(
        "household_rows",
        household_rows_started_at,
        household_rows_started_queries,
        row_count=len(household_rows),
        record_count=len(records),
    )

    grouped_records = itertools.groupby(
        sorted(records, key=lambda x: (x["Household_number"])),
        key=lambda x: (x["Household_number"]),
    )
    ...
    _log_analysis_timing(
        "covid_and_toilet_rows",
        covid_rows_started_at,
        covid_rows_started_queries,
        covid_count=len(covid_rows),
        toilet_count=len(Toilet_data),
    )
    cpod_status = [
        "SBM (Installment)",
        "SBM (Contractor)",
    ...
    for household, list_record in grouped_records:
        ...
        if (
            "Type_of_structure_occupancy" in record
            and record["Type_of_structure_occupancy"] != "Shop"
        ):
            if household_no_key in covid_hh:
                ...
                if (
                    full_vaccinated_counts.get(household_no_key, 0) == cnt_mem
                    and cnt_mem
                ):
        ...
        if (
            "group_oi8ts04/Current_place_of_defecation" in record
            and record["group_oi8ts04/Current_place_of_defecation"] in cpod_status[:4]
            and slum_code != 1971
        ):
        ...
        if household_no_key in Toilet_data:
            if (
                "group_oi8ts04/Current_place_of_defecation" in record
                and record["group_oi8ts04/Current_place_of_defecation"] in cpod_status
                and slum_code != 1971
            ):
                record["group_oi8ts04/Current_place_of_defecation"] = "Use CTB"
        ...
        if "group_oi8ts04/Current_place_of_defecation" in record:
            if (
                record["group_oi8ts04/Current_place_of_defecation"] in cpod_status
                or "Final_status" in record
            ):
                ...
                record["remain_ctb"] = (
                    "available"  # Checking remining CTB's for POST SBM
                )

            if (
                "group_oi8ts04/Are_you_interested_in_an_indiv" in record
                and record["group_oi8ts04/Are_you_interested_in_an_indiv"] == "Yes"
            ):
                record["remain_intrested"] = (
                    "Yes"  # Checking remining Interested for POST SBM
                )
        ...
        for field in question_fields:
            ...
            if field != "" and field in record and record[field] == record[field]:
                ...
                if (
                    "Type_of_structure_occupancy" in record
                    and record["Type_of_structure_occupancy"] != "Occupied house"
                ):
                    if field in [
                ...
                if field in output:
                    if "data_not_available" in output[field]:
                        ...
                        output[field]["data_not_available"] = [
                            household_no_key,
                        ]
                else:
                    output[field] = {
...
def format_data(rhs_data, slum_id, toilet_by_sa=False):
    ...
    for k, v in seq.items():
        try:
            ...
            if (
                k == "group_oi8ts04/Current_place_of_defecation"
            ):  # Changing cpod status and adding new cpod for toilet by sa households.
                if toilet_by_sa:
                    new_rhs[v] = "Toilet By SA"
                    new_rhs["Before Home Toilet Place of defication"] = rhs_data[k]
                    if (
                        "group_oi8ts04/Are_you_interested_in_an_indiv" in rhs_data
                    ):  # Removing household from intrested for toilet if household have toilet.
                        del rhs_data["group_oi8ts04/Are_you_interested_in_an_indiv"]
                elif rhs_data[k] in cpod_status:
                    new_rhs["Before Home Toilet Place of defication"] = (
                        "Use CTB"  #  If the hh has Toilet by sbm then cpod is Use CTB.
                    )
                ...
                new_rhs[v] = (
                    (rhs_data[k] + " " + rhs_data["Plus Code Part"])
                ...
                if k in rhs_data:
                    value = rhs_data[k]
                    new_rhs[v] = rhs_data[k]
                    if v == "Contact number" and (
                        not value or str(value).strip() == "0"
                    ):
...
def get_kobo_RHS_list(city, slum, slum_id, house_number, kobo_survey=""):
    ...
    output = OrderedDict()
    household_data = HouseholdData.objects.filter(
        slum=slum, household_number=house_number
    ).order_by("submission_date")
    Toilet_data = list(
        ToiletConstruction.objects.filter(slum=slum, status=6).values_list(
    ...
    if len(household_data) > 0:
        ...
        logger.warning(
            "No RHS data found for Slum: %s and Household Number: %s",
            slum,
            house_number,
        )
    return output
...
def getPlusCodeDetails(slum, household):
    rhs_obj = HouseholdData.objects.filter(
        slum=slum, household_number=str(int(household)), rhs_data__isnull=False
    )
    if slum == "1971":
    ...
    if rhs_obj.exists():
        ...
        if "Plus code of the house" in rhs_data:
            if "Plus Code Part" in rhs_data:
                pluscode = (
                    rhs_data["Plus code of the house"]
                    + "-"
                    + rhs_data["Plus Code Part"]
                )
                return pluscode
...
def get_kobo_RIM_detail(city, slum_code, kobo_survey=""):
    ...
    submission = SlumData.objects.filter(slum_id=slum[0].id).values_list(
        "rim_data", flat=True
    )[0]
    if len(submission) > 0:
...
def parse_RIM_data(submission):
    ...
    match_keys = {
        "Drainage": {
            "coverage_of_drains_across_the": "Coverage of drains across the settlement",
        ...
        "Road": {
            "are_the_huts_below_or_above_th": "Are the huts below or above the internal access road",
        ...
        "Toilet": {
            "Out_of_total_seats_o_of_pans_not_choked": "Out of total seats, no of pans not choked",
        ...
        "Waste": {
            "coverage_of_waste_collection_a_001": "Coverage of door to door  waste collection",
        ...
        "Water": {
            "Total_number_of_standposts_NOT": "Total number of standposts NOT in use",
    ...
    for key, value in submission.items():
        ...
        if key != "Toilet":  # here we are iterating on all the non repetative section.
            match_keys_dict = match_keys[
                key
            ]  # here we are matching the section key to actual name of the question.
            ...
            match_keys_dict = match_keys[
                key
            ]  # here we are iterating repetative section Toilet section
            cnt = 1
            output[key] = []
            for i in value:
                data1 = OrderedDict()
                for key1, value1 in i.items():
                    if key1 in match_keys_dict:
                        data1[match_keys_dict[key1]] = (
                            value1  # here we are matching the section key to actual name of the question.
                        )
...
def get_kobo_RIM_report_detail(city, slum_code, kobo_survey=""):
    ...
    submission_date, created_on, modified_on = rim_qs.values_list(
        "submission_date", "created_on", "modified_on"
    )[0]
    output["submission_date"] = (
        submission_date.strftime("%B %Y") if submission_date else "NA"
    )
    ...
    def dict_to_str(data_dict):
        try:
            ...
            if len(data_dict.keys()) > 1:
                ...
                result_str += (
                    "," + data_dict_key + "(" + str(data_dict[data_dict_key]) + ")"
                )
            return result_str
        except Exception as e:
            logger.error(e)
    ...
    if "Toilet" in data:
        ...
        for i in toilet_info:
            ...
            if "is_the_CTB_in_use" in i and i["is_the_CTB_in_use"] == "Yes":
                ...
                water_supply_condition.extend(
                    i["type_of_water_supply_in_ctb"].split(",")
                )
            ...
            water_supply_condition_dict = {
                i: water_supply_condition.count(i) for i in water_supply_condition
            }

            dict_to_str_keys = {
                "ctb_maintenance_provided_by": ctb_maintenance_provider,
                "condition_of_ctb_structure": ctb_structure_condition,
                "cleanliness_of_the_ctb": ctb_cleanliness_condition,
                "type_of_water_supply_in_ctb": water_supply_condition_dict,
                "facility_in_the_toilet_block_f": ctb_for_under_5,
                "sewage_disposal_system": ctb_sewage_disposal_system,
            }
        ...
        output["number_of_seats_allotted_to_wo"] = output[
            "total_number_of_mixed_seats_al"
        ] = output["number_of_seats_allotted_to_me"] = 0
        ...
        if data["type"] == "group":
            ...
            for sub_k in toil_keys:
                if type(submission[0][sub_k]) == list:
                    ...
                    sub_key.extend(
                        sum([list(k.keys()) for k in submission[0][sub_k]], [])
                    )
                else:
    ...
    for data in data1["children"]:
        if data["type"] == "group" and data["name"] in section.keys():
            ...
            if data["name"] != RIM_TOILET:
                ...
                [
                    output[section[data["name"]]].append(OrderedDict())
...
def get_kobo_FF_report_detail(city, slum_code, house_number, kobo_survey=""):
    ...
    if kobo_survey:
        householdData = HouseholdData.objects.filter(
            slum__shelter_slum_code=slum_code, household_number=str(house_number)
            ...
            if "_attachments" in output and len(output["_attachments"]) != 0:
                ...
                for photo in output["_attachments"]:
                    if (
                        ...
                        and output["Toilet_Photo"] in photo["filename"]
                    ):
                        output["Toilet_Photo"] = PATH + "/" + output["Toilet_Photo"]
                    if (
                        "Family_Photo" in output
                        and output["Family_Photo"] in photo["filename"]
                    ):
...
def get_kobo_FF_report_detail1(city, slum_code, house_number, kobo_survey=""):
    ...
    if kobo_survey:
        try:
            url = (
                settings.KOBOCAT_FORM_URL
                + "data/"
                + kobo_survey
                + '?format=json&query={"group_vq77l17/slum_name":"'
                + slum_code
                + '","group_vq77l17/Household_number":{"$in":["'
                + house_number
                + '","'
                + ("000" + house_number)[-4:]
                + '"]}}'
        ...
        if len(submission) > 0:
            for data in data1["children"]:
                if (
                    ...
                    sect_form_data = trav(data)
                    sub_key = [
                        str(k) for k in submission[0].keys() if data["name"] in k
                    ] + ["_attachments"]
                    for sect_form in sect_form_data:
                        output[sect_form["name"]] = ""
                        key = [x for x in sub_key if x.endswith(sect_form["name"])]
...
def fetch_answer(sect_form, key, submission):
    ...
    val = ""
    if (
        "select" in sect_form["type"]
        and type(key[0]) != list
        and "children" in sect_form
    ):
        options = dict(
            (str(opt["name"]), str(opt["label"])) for opt in sect_form["children"]
        )
        ...
        val = [
            photo["download_url"]
            for photo in photos
            if submission[key[0]] in photo["filename"]
        ]
