...
class DDSync(object):
    def __init__(self, slum, user):
        ...
        sync_info = (
            KoboDDSyncTrack.objects.filter(slum=self.slum)
            .order_by("-sync_date")
            .first()
        )
        if sync_info:
            self.survey_date = self.convert_datetime(
                str(timezone.localtime(sync_info.sync_date))
            )
    ...
    def fetch_url(self, community_mobilization_flag):
        ...
        kobo_url += (
            '?format=json&query={"slum_name":"' + self.slum.shelter_slum_code + '"'
        )
        kobo_url += (
            ',"Was_any_Community_Mobilisation":"1" '
            if community_mobilization_flag
            else ""
        )
    ...
    def fetch_kobo_data(self, community_mobilization_flag=False):
        url = self.fetch_url(community_mobilization_flag) % (
            str(self.survey_date).replace(" ", "T")
        )
        self.survey_record = self.fetch_url_data(url)

    def fetch_kobo_FF_data(self):
        url_family_factsheet = (
            settings.KOBOCAT_FORM_URL
            + "data/"
            + str(self.ff_survey_id)
            + "?format=json"
        )
        url_family_factsheet += (
            '&query={"group_vq77l17/slum_name":"'
            + str(self.slum.shelter_slum_code)
            + '"'
        )
    ...
    def update_sync_info(self, sync_date):
        try:
            KoboDDSyncTrack.objects.create(
                slum=self.slum, sync_date=sync_date, created_by=self.user
            )
        except Exception as e:
            logger.error("Error creating record" + str(e))
...
class ToiletConstructionSync(DDSync):
    ...
    def __init__(self, slum, user):
        ...
        self.MAPPING = {
            "House_numbers_of_hou_Septic_Tank_is_given": "septic_tank_date",
            "House_numbers_of_hou_re_agreement_is_done": "agreement_date",
            "House_numbers_of_hou_and_cement_is_given": "phase_one_material_date",
            "House_numbers_of_hou_al_Hardware_is_given": "phase_two_material_date",
            "House_numbers_of_hou_HASE_3_Door_is_given": "phase_three_material_date",
            "House_numbers_of_hou_truction_is_complete": "completion_date",
        ...
        self.REGX_CHECK = {
            "House_numbers_where_ifted_from_HH_to_HH": "p1_material_shifted_to",
            "House_numbers_where_ifted_from_002": "p2_material_shifted_to",
            "House_numbers_where_ifted_from_001": "p3_material_shifted_to",
            "House_numbers_where_ifted_from": "st_material_shifted_to",
        }
    ...
    def fetch_FF_data(self):
        ...
        for tmp_ff in formdict_family_factsheet:
            ...
            check_list = {
                "slum": self.slum,
                "household_number": tmp_ff["group_vq77l17/Household_number"],
            }
            tc, created = ToiletConstruction.objects.get_or_create(**check_list)
            if "group_ne3ao98/Use_of_toilet" in tmp_ff and tmp_ff[
                "group_ne3ao98/Use_of_toilet"
            ] in ["01", "02", "03", "04", "05", "06"]:
                update_data["use_of_toilet"] = self.convert_datetime(
                    tmp_ff["_submission_time"]
                )
            if (
                "group_ne3ao98/Where_the_individual_ilet_is_connected_to" in tmp_ff
                and tmp_ff["group_ne3ao98/Where_the_individual_ilet_is_connected_to"]
                in ["01", "03"]
            ):
                update_data["toilet_connected_to"] = self.convert_datetime(
                    tmp_ff["_submission_time"]
                )
            if not tc.factsheet_done:
                update_data["factsheet_done"] = self.convert_datetime(
                    tmp_ff["_submission_time"]
                )
        ...
        for record in self.survey_record:
                ...
                if kobofield in record:
                    ...
                    for house in houses:
                        ...
                        if len(house_data[0][0]) < 5:
                            check_list = {
                                "slum": self.slum,
                                "household_number": house_data[0][0],
                            }
                            tc, created = ToiletConstruction.objects.get_or_create(
                                **check_list
                            )
                            update_data = {}
                            update_data[modelfield] = house_data[0][1]
                            tc = ToiletConstruction.objects.filter(**check_list)
                            ToiletConstruction.objects.filter(**check_list).update(
                                **update_data
                            )
            ...
            for kobofield, modelfield in self.MAPPING.items():
                if kobofield in record:
                    ...
                    for house in houses:
                        if len(house) < 5:
                            tc = ToiletConstruction.objects.get_or_create(
                                slum=self.slum, household_number=house
                            )
                    flag = True
...
class CommunityMobilizaitonSync(DDSync):
    ...
    def fetch_data(self):
        ...
        for record in self.survey_record:
            ...
            cm, created = CommunityMobilization.objects.get_or_create(
                slum=self.slum, activity_type=activity_type, activity_date=activity_date
            )

            for house in houses.split(","):
                if len(house) < 5:
                    flag = True
                    com_mobs = CommunityMobilization.objects.filter(
                        slum=self.slum,
                        activity_type=activity_type,
                        household_number__contains=house,
                    )
                    if len(com_mobs) <= 0:
