...
class VendorHouseholdInvoiceDetail(models.Model):
    ...
    def __str__(self):
        return (
            self.vendor.name + " - " + self.slum.name + "(" + self.invoice_number + ")"
        )
...
class Invoice(models.Model):
    ...
    created_by = models.ForeignKey(
        User, related_name="invoice_created_by", on_delete=models.DO_NOTHING
    )
    modified_on = models.DateTimeField(default=datetime.datetime.now)
    modified_by = models.ForeignKey(
        User, related_name="invoice_modified_by", on_delete=models.DO_NOTHING
    )
...
UNITS = (("1", "Tempo/Piago"), ("2", "Bags"), ("3", "Nos"))
PHASE = (("1", "Phase One"), ("2", "Phase Two"), ("3", "Phase Three"))
...
class InvoiceItems(models.Model):
    ...
    created_on = models.DateTimeField(default=datetime.datetime.now)
    created_by = models.ForeignKey(
        User, related_name="invoiceitem_created_by", on_delete=models.DO_NOTHING
    )
    modified_on = models.DateTimeField(default=datetime.datetime.now)
    modified_by = models.ForeignKey(
        User, related_name="invoiceitem_modified_by", on_delete=models.DO_NOTHING
    )
...
class ToiletConstruction(models.Model):
    ...
    status = models.CharField(
        choices=STATUS_CHOICES, max_length=2, null=True, blank=True
    )
    ...
    def update_model(self, df1):
        ...
        if not self.agreement_date:
            self.agreement_date = (
                df1.loc["Date of Agreement"]
                if pandas.isnull(df1.loc["Date of Agreement"]) is False
                else None
            )

        if not self.septic_tank_date:
            self.septic_tank_date = (
                df1.loc["Date of Septic Tank supplied"]
                if pandas.isnull(df1.loc["Date of Septic Tank supplied"]) is False
                else None
            )

        if not self.phase_one_material_date:
            self.phase_one_material_date = (
                df1.loc["Material Supply Date 1st"]
                if pandas.isnull(df1.loc["Material Supply Date 1st"]) is False
                else None
            )

        if not self.phase_two_material_date:
            self.phase_two_material_date = (
                df1.loc["Material Supply Date-2nd"]
                if pandas.isnull(df1.loc["Material Supply Date-2nd"]) is False
                else None
            )

        if not self.phase_three_material_date:
            self.phase_three_material_date = (
                df1.loc["Material Supply Date-3rd"]
                if pandas.isnull(df1.loc["Material Supply Date-3rd"]) is False
                else None
            )

        if not self.completion_date:
            self.completion_date = (
                df1.loc["Construction Completion Date"]
                if pandas.isnull(df1.loc["Construction Completion Date"]) is False
                else None
            )

        if not self.pocket:
            self.pocket = (
                df1.loc["Pocket"] if pandas.isnull(df1.loc["Pocket"]) is False else None
            )
...
def update_status(sender, instance, **kwargs):
    ...
    if instance.status != STATUS_CHOICES[6][0]:
        ...
        if (
            (instance.phase_one_material_date is None)
            and instance.agreement_date
            and (
                datetime.date.today() - instance.agreement_date
        ...
        if (
            instance.phase_one_material_date
            or instance.phase_two_material_date
            or instance.phase_three_material_date
        ):
            instance.status = STATUS_CHOICES[4][0]  # Under Construction
...
def handle_shifted_material(sender, instance, **kwargs):
    ...
    if instance.p1_material_shifted_to is not None:
        flag = True
        try:
            hh_does_exist = ToiletConstruction.objects.get(
                slum=instance.slum, household_number=instance.p1_material_shifted_to
            )
        ...
        if (
            flag
            and len(instance.p1_material_shifted_to) != 0
            and instance.phase_one_material_date is not None
        ):
            temp["flag"] = True
    ...
    if instance.p2_material_shifted_to is not None:
        ...
        try:
            hh_does_exist = ToiletConstruction.objects.get(
                slum=instance.slum, household_number=instance.p2_material_shifted_to
            )
        ...
        if (
            flag
            and len(instance.p2_material_shifted_to) != 0
            and instance.phase_two_material_date is not None
        ):
            temp["flag"] = True
            temp["household_number"] = int(instance.p2_material_shifted_to)
            temp["defaults"][
                "phase_two_material_date"
            ] = instance.phase_two_material_date
    ...
    if instance.p3_material_shifted_to is not None:
        ...
        try:
            hh_does_exist = ToiletConstruction.objects.get(
                slum=instance.slum, household_number=instance.p3_material_shifted_to
            )
        ...
        if (
            flag
            and len(instance.p3_material_shifted_to) != 0
            and instance.phase_three_material_date is not None
        ):
            temp["flag"] = True
            temp["household_number"] = int(instance.p3_material_shifted_to)
            temp["defaults"][
                "phase_three_material_date"
            ] = instance.phase_three_material_date
    ...
    if instance.st_material_shifted_to is not None:
        ...
        try:
            hh_does_exist = ToiletConstruction.objects.get(
                slum=instance.slum, household_number=instance.st_material_shifted_to
            )
        ...
        if (
            flag
            and len(instance.st_material_shifted_to) != 0
            and instance.septic_tank_date is not None
        ):
            temp["flag"] = True
    ...
    if temp["flag"]:
        ...
        if (
            (instance.phase_one_material_date is None)
            and (instance.phase_two_material_date is None)
            and (instance.phase_three_material_date is None)
            and instance.agreement_date
            and (
                datetime.date.today() - instance.agreement_date
                > datetime.timedelta(days=8)
...
def check_duplicate_house(sender, instance, **kwargs):
    ...
    if instance.id is not None:
        all_records = InvoiceItems.objects.filter(slum=instance.slum).exclude(
            id=instance.id
        )
    else:
    ...
    for record in all_records:
        if record.material_type == instance.material_type:
            common_households = list(
                set(record.household_numbers).intersection(instance.household_numbers)
            )
            if len(common_households) != 0:
                errors.append(
                    ...
                    + str(record.slum.id)
                    + "), household numbers "
                    + str(common_households)
                    + " are repeated in "
                    + str(record.invoice)
                    + " and "
                    + str(instance.invoice)
                )
