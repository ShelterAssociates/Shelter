...
from mastersheet.daily_reporting_sync import (
    ToiletConstructionSync,
    CommunityMobilizaitonSync,
)
...
def give_details(request):
    ...
    try:
        delimiter = "&"
        slum_code = Slum.objects.filter(
            pk=int(request.GET.get("form").split("&")[1].split("=")[1])
        ).values_list(
            "shelter_slum_code",
        ...
        slum_info_dict.update(
            {
                "Name of the slum": slum_code[0][3],
                "Electoral Ward": slum_code[0][2],
                "City Code": slum_code[0][1],
            }
        )
    ...
    try:
        ...
        if slum_code is not 0:
            if flag_fetch_rhs:
                ...
                def check_rhs_data(record):
                    ...
                    if (
                        ...
                        data = {
                            rhs_key: record.rhs_data[rhs_key]
                ...
                formdict = list(
                    map(
                        lambda x: {
                            "Household_number": x.household_number,
                            "id": x.id,
                            "rhs_uuid": (
                                x.rhs_data["rhs_uuid"]
                                if "rhs_uuid" in x.rhs_data
                                else None
                            ),
                        },
            ...
            toilet_reconstruction_fields = [
                ...
                "agreement_date_str",
                "agreement_cancelled",
                "septic_tank_date_str",
                "phase_one_material_date_str",
                "phase_two_material_date_str",
                "phase_three_material_date_str",
                "completion_date_str",
                "status",
                "comment",
                "pocket",
                "p1_material_shifted_to",
                "p2_material_shifted_to",
            ...
            daily_reporting_data = ToiletConstruction.objects.extra(
                select={
                    "phase_one_material_date_str": "to_char(phase_one_material_date, 'YYYY-MM-DD ')",
                    ...
                    "completion_date_str": "to_char(completion_date, 'YYYY-MM-DD ')",
                }
            ...
            for i in daily_reporting_data:
                if i["status"] is not None:
                    if i["status"].strip() != "":
                        ...
                        i["delay_flag"] = ""
                if (
                    i["agreement_date_str"] != None
                    and i["status"] != "Agreement cancel"
                ):
                    if i["phase_one_material_date_str"] == None and is_delayed(
                        i["agreement_date_str"]
                    ):
                        i["delay_flag"] = "#f9a4a4"  # phase one delayed
                    if i["phase_two_material_date_str"] == None and is_delayed(
                        i["phase_one_material_date_str"]
                    ):
                        i["delay_flag"] = "#f2f29f"  # phase two delayed
                    if (
                        i["phase_one_material_date_str"]
                        != i["phase_three_material_date_str"]
                    ):
                        if i["phase_three_material_date_str"] == None and is_delayed(
                            i["phase_two_material_date_str"]
                        ):
                            i["delay_flag"] = "#aaf9a4"
                        if i["completion_date_str"] == None and is_delayed(
                            i["phase_three_material_date_str"]
                        ):
                            i["delay_flag"] = "#aaa4f4"
                    else:
            ...
            sbm_data = SBMUpload.objects.extra(
                select={"created_date_str": "to_char(created_date, 'YYYY-MM-DD ')"}
            ).filter(slum__id=slum_code[0][0])
            ...
            community_mobilization_data = CommunityMobilization.objects.extra(
                select={"activity_date_str": "to_char(activity_date, 'YYYY-MM-DD ')"}
            ).filter(slum__id=slum_code[0][0])
            community_mobilization_data_avni = list(
                CommunityMobilizationActivityAttendance.objects.filter(
            ...
            for y in community_mobilization_data_avni:
                hh = normalize_household_number(y.household_number)
                new_activity_type = y.activity_type.name
                if hh in dummy_formdict.keys():
                    dummy_formdict[hh].update(
                        {
            ...
            for y in community_mobilization_data:
                if y.household_number != None:
                    ...
                    for z in y.household_number:
                        ...
                        dummy_formdict[z].update(
                            {
                                new_activity_type: y.activity_date_str,
                                str(new_activity_type) + "_id": y.id,
                            }
                        )
            ...
            for i in temp_sbm_keys:
                ...
                dummy_formdict[i].update(
                    {"sbm_id_" + i: temp_sbm[i]["id"]}
                )
            ...
            for i in temp_DR_keys:
                ...
                dummy_formdict[i].update(
                    {"tc_id_" + i: temp_daily_reporting[i]["id"]}
                )
            ...
            for key, x in dummy_formdict.items():
                try:
                    if (
                        len(x["p1_material_shifted_to"]) != 0
                        or len(x["p2_material_shifted_to"]) != 0
                        or len(x["p3_material_shifted_to"]) != 0
                        or len(x["st_material_shifted_to"]) != 0
                    ):
                ...
                if flag_fetch_ff:
                    ...
                    if "_attachments" in x.keys() and len(x["_attachments"]) != 0:
                        ...
                        if "Toilet_Photo" and "ff_uuid" in x.keys():
                            ff_url = (
                                settings.AVNI_URL
                                + "#/app/subject/viewProgramEncounter?uuid="
                                + str(x["ff_uuid"])
                            )
                            x.update(
                                {
                                    "toilet_photo_url": (
                                        ff_url if ff_url else x["Toilet_Photo"]
                                    )
                                }
                            )
                            x.update(
                                {
                                    "family_photo_url": (
                                        ff_url if ff_url else x["Family_Photo"]
                                    )
                                }
                            )
                ...
                if flag_fetch_rhs:
                    ...
                    if "status" in x and x["status"] == "Completed":
                        ...
                        x.update(
                            {
                                "group_oi8ts04/Which_Community_Toil_r_family_members_use": ""
                            }
                        )
            ...
            for x in formdict:
                try:
                    if (
                        "current place of defecation" in x
                        and x["current place of defecation"]
                        in ["SBM (Installment)", "Own toilet"]
                    ) and (
                        "agreement_date_str" in x
                        and x["agreement_date_str"]
                        and len(x["agreement_date_str"]) > 1
                    ):
...
def trav(node):
    ...
    if "type" in node and node["type"] == "group":
        ...
        node["type"] == "select one" or node["type"] == "select all that apply"
    ) and "children" in node.keys():
        return [node]
    return []
...
def define_columns(request):
    ...
    formdict_new = [
        ...
        {
            "data": "group_el9cl08/Enter_the_10_digit_mobile_number",
            "title": "Mobile number",
        },
        {"data": "group_el9cl08/Aadhar_number", "title": "Aadhar card number"},  # 10
        {
            "data": "group_el9cl08/Number_of_household_members",
        ...
        {
            "data": "group_el9cl08/Type_of_structure_of_the_house",
            "title": "Type of structure of the house",
        },
        {
            "data": "group_el9cl08/Ownership_status_of_the_house",
            "title": "Ownership status of the house",
        },  # 15
        {"data": "group_el9cl08/House_area_in_sq_ft", "title": "House area in sq. ft."},
        {
            "data": "group_el9cl08/Type_of_water_connection",
            "title": "Type of water connection",
        },
        {
            "data": "group_el9cl08/Facility_of_solid_waste_collection",
            "title": "Facility of solid waste management",
        },
        {
        ...
        {
            "data": "group_oi8ts04/How_many_installments_have_you",
            "title": "How many installments have you received?",
        },
        {
        ...
        {
            "data": "group_oi8ts04/Status_of_toilet_under_SBM",
            "title": "Status of toilet under SBM?",
        },
        {
        ...
        {
            "data": "group_oi8ts04/Which_Community_Toil_r_family_members_use",
            "title": "Which CTB",
        },  # 29
        {
        ...
        {
            "data": "group_oi8ts04/What_kind_of_toilet_would_you_likes",
            "title": "What kind of toilet would you like?",
        },
        {
        ...
        {
            "data": "group_oi8ts04/What_is_the_toilet_connected_to",
            "title": "What is the toilet connected to?",
        },  # 35
        {
            "data": "group_oi8ts04/Who_all_use_toilets_in_the_hou",
            "title": "Who all use toilets in the household?",
        },
        {
            "data": "group_oi8ts04/Reason_for_not_using_toilet",
            "title": "Reason for not using toilet",
        },
        ...
        {
            "data": "group_ne3ao98/Cost_of_upgradation_in_Rs",
            "title": "House renovation cost",
        },  # 40
        {
        ...
        {
            "data": "phase_one_material_date_str",
            "title": "Date of first phase material",
        },
        {
            "data": "phase_two_material_date_str",
            "title": "Date of second phase material",
        },
        {
            "data": "phase_three_material_date_str",
            "title": "Date of third phase material",
        },  # 60
        {"data": "completion_date_str", "title": "Construction Completion Date"},
        {
            "data": "p1_material_shifted_to",
            "title": "Phase one material shifted to",
        },  ##62
        {
            "data": "p2_material_shifted_to",
            "title": "Phase two material shifted to",
        },  ##63
        {
            "data": "p3_material_shifted_to",
            "title": "Phase three material shifted to",
        },  ##64
    ...
    activity_type_model = ActivityType.objects.filter(display_flag=True).order_by(
        "display_order"
    )
    try:
        for i in range(len(activity_type_model)):
            formdict_new.append(
                {
                    "data": activity_type_model[i].name,
                    "title": activity_type_model[i].name,
                }
            )
    except Exception as e:
        logger.error(e)
    final_data["buttons"]["Community Mobilization"] = list(
        range(activity_pre_len, len(formdict_new))
    )

    material_type_model = MaterialType.objects.filter(display_flag=True).order_by(
        "display_order"
    )
    ...
    try:
        for i in material_type_model:
            formdict_new.append(
                {
                    "data": "invoice_number" + str(i.name),
                    "title": str(i.name) + " Invoice Number",
                }
            )
    except Exception as e:
        logger.error(e)
                        ...
                        if TC_instance:
                            ...
                            for j in range(len(STATUS_CHOICES)):
                                if (
                                    ...
                                    == str(stat).lower()
                                ):  # STATUS_CHOICE is imported from mastersheet/models.py
...
def delete_selected_records(records):
    ...
    form_ids = Survey.objects.filter(
        city=slum_code.electoral_ward.administrative_ward.city,
    ...
    for r in records["records"]:
        try:
            if r:
                deleteURL = "/".join(
                    [settings.KOBOCAT_FORM_URL[:-1], "data", str(kobo_form), str(r)]
                )
                objresponseDeleted = requests.delete(deleteURL, headers=headers)
        except Exception as e:
            logger.error("No record selected to delete.")
...
def sync_kobo_data(request):
    ...
    try:
        ...
        if not (slum_in_avni_flag):
            data["msg"] = (
                "The Avni UUID mapping is not available for this slum."
                + slum.name
            ...
            if rim_sync_num > 0:
                ...
                data["msg"] = (
                    "There is no records available to sync RIM data for - " + slum.name
                )
                data["msg"] += "\nTotal records updated : " + str(rim_sync_num)
            if toilet_sync_num > 0:
                ...
                data["msg"] += (
                    "\n There is no records available to sync CTB data for slum - "
                    + slum.name
                )
                data["msg"] += "\nTotal records updated : " + str(toilet_sync_num)
            if rim_add_flag:
        ...
        data["msg"] = (
            "Error occurred while sync from kobo. Please contact administrator."
            + str(e)
        )
    return HttpResponse(
...
def create_report(request):
    ...
    cities = City.objects.filter(name__city_name__in=group_perm).values(
        "id", "name__city_name"
    )
    for i in cities:
        temp = {}
        ...
        temp["id"] = i["id"]
        temp["children"] = list(
            AdministrativeWard.objects.filter(city=i["id"]).values("id", "name")
        )
        temp["tag"] = "city"
        for j in temp["children"]:
            j["tag"] = "admin_ward"
            j["children"] = list(
                ElectoralWard.objects.filter(administrative_ward=j["id"]).values(
                    "id", "name"
                )
            )
            for k in j["children"]:
                ...
                k["children"] = list(
                    Slum.objects.filter(electoral_ward=k["id"]).values("id", "name")
                )
...
level_data = {
    "city": {
        "city_name": F(
            "slum__electoral_ward__administrative_ward__city__name__city_name"
        ),
...
def report_table_cm(request):
    ...
    for x in activity_type:
        ...
        yy = (
            x.communitymobilizationactivityattendance_set.filter(**filter_field_new)
            .annotate(**level_data[tag])
            .values("level", "level_id", "city_name")
            .annotate(**count_field1)
            .order_by("city_name")
        )
        ...
        for data in yy:
            ...
            if (
                str(level_id) in report_table_data_cm.keys()
                and key_for_datatable in report_table_data_cm[str(level_id)].keys()
            ):
                report_table_data_cm[str(level_id)][key_for_datatable] += data[
                    key_for_datatable
                ]
            else:
    ...
    return HttpResponse(
        json.dumps(list(map(lambda x: report_table_data_cm[x], report_table_data_cm))),
        content_type="application/json",
    )
...
def report_table_cm_activity_count(request):
    ...
    for x in activity_type:
        ...
        yy = (
            ...
            .values("level", "level_id", "city_name")
            .annotate(**count_field)
            .order_by("city_name", "date_of_activity")
        )

        for data in y:
            level_id = data["level_id"]
            if (
                str(level_id) in report_table_data_cm_activity_count.keys()
                and key_for_datatable
                in report_table_data_cm_activity_count[str(level_id)].keys()
            ):
                report_table_data_cm_activity_count[str(level_id)][
                    key_for_datatable
                ] += data[key_for_datatable]
            else:
    ...
    return HttpResponse(
        json.dumps(
            list(
                map(
                    lambda x: report_table_data_cm_activity_count[x],
                    report_table_data_cm_activity_count,
                )
            )
        ),
        content_type="application/json",
...
def give_report_table_numbers_accounts(request):
    ...
    query_on = {
        "phase_one_material_date": "total_date_phase_1",
            ...
            if tc_result_one.count() > 0:
                for tc_obj in tc_result_one:
                    if tc_obj["level_id"] in report_table_accounts_data:
                        if (
                            query_on[query_field]
                            in report_table_accounts_data[tc_obj["level_id"]]
                        ):
                            report_table_accounts_data[tc_obj["level_id"]][
                                query_on[query_field]
                            ] += tc_obj[query_on[query_field]]
                        else:
                            report_table_accounts_data[tc_obj["level_id"]][
    ...
    invoiceItems = (
        InvoiceItems.objects.filter(**filter_field)
        .annotate(**level_data[tag])
        .values(
            "level",
            "level_id",
    ...
    invoiceItems = itertools.groupby(
        sorted(invoiceItems, key=lambda x: x["level_id"]), key=lambda x: x["level_id"]
    )
    ...
    for level, level_wise_list in invoiceItems:
        ...
        phasewise = itertools.groupby(
            sorted(level_wise_list, key=lambda x: x["phase"]), key=lambda x: x["phase"]
        )
        level_name = None
        city_name = None
        phase_wise_material_cnt = (
            {}
        )  # Here we are checking slum wise households count for each phase.
        for phase, phasewisedata in phasewise:
            ...
            phasecnt = sum(
                list(map(len, slum_wise_acc_data.values()))
            )  # Generating total count of households phase wise
            phase_wise_material_cnt["total_material_phase_" + str(phase)] = phasecnt
        phase_wise_material_cnt.update(
            {"level": level_name, "level_id": level, "city_name": city_name}
        )  # Adding Level details and citu names for cases where we have accounts data but not daily reporting.
        report_table_accounts_data[level].update(
            phase_wise_material_cnt
        )  # adding this count into final response data level wise.
    return HttpResponse(
        json.dumps(
            list(
                map(lambda x: report_table_accounts_data[x], report_table_accounts_data)
            )
        ),
        content_type="application/json",
    )
...
def accounts_excel_generation(request):
    ...
    if (
        len(request.POST.get("account_start_date")) == 0
        or len(request.POST.get("account_end_date")) == 0
    ):
        start_date = datetime.datetime(2001, 1, 1).date()
        end_date = datetime.datetime.today().date()
    else:
        start_date = datetime.datetime.strptime(
            request.POST.get("account_start_date"), "%d-%m-%Y"
        ).date()
        end_date = datetime.datetime.strptime(
...
def renderSummery(request):
    ...
    return render(
        request,
        "mastersheet_summery.html",
        {"form": slum_search_field, "form_gis": gis_field, "form_sponsor": sponsor},
    )
...
def getRhsData(record):
    key_list = {
        ...
        "group_el9cl08/Ownership_status_of_the_house": "ownership_status",
        "group_oi8ts04/Current_place_of_defecation": "current_place_of_defication",
        "Plus Code Part": "pluscodepart",
        "group_el9cl08/Do_you_have_any_girl_child_chi": "girls_child",
        "group_el9cl08/Type_of_structure_of_the_house": "house_structure",
        "group_el9cl08/Ownership_status_of_the_house": "ownership_status",
        "group_el9cl08/House_area_in_sq_ft": "house_area",
        "Do you have addhar card?": "isAadharCard",
        "group_el9cl08/Aadhar_number": "aadhar_number",
        "Colour of ration card": "color_of_ration_card",
        "Select Religion": "Select Religion",
        "If other religion": "If other religion",
        "Select Caste": "Select Caste",
        "If other caste": "If other caste",
        "HH_can_connect_to_Drainage": "HH can connect to Drainage line",
        "group_el9cl08/Type_of_water_connection": "Type_of_water_connection",
        "group_el9cl08/Facility_of_solid_waste_collection": "Facility_of_solid_waste_collection",
    ...
    if (
        ...
        if (
            "Type_of_water_connection" in data
            and data["Type_of_water_connection"] == "Individual connection"
        ):
        ...
        if "Electricity_data" in record.rhs_data:
            ...
            electricity_data = {
                key_list[key]: electricity_data_obj[key]
        ...
        key_list = {
            "Plus code of the house": "pluscodes",
            "Type_of_structure_occupancy": "occupancy_status",
            "Type_of_unoccupied_house": "typeUnoccupiedHouse",
            "HH_can_connect_to_Drainage": "HH can connect to Drainage line",
        }
        data = {
            key_list[i]: record.rhs_data[i]
    ...
    if record.ff_data:
        factsheet_keys = {
            ...
            "group_oh4zf84/Name_of_the_family_head": "family_factsheet_name",
            "group_ne3ao98/Where_the_individual_ilet_is_connected_to": "toilet_connected_to",
        }
    ...
    data["Last Modified At"] = str(
        datetime.datetime.strptime(str(record.submission_date)[:10], "%Y-%m-%d").date()
    )
...
def communityActivityData(slum_code):
    act_data = CommunityMobilization.objects.filter(
        slum_id=slum_code[0][0]
    ).values_list("activity_type", "household_number")
    comm_avni = CommunityMobilizationActivityAttendance.objects.filter(
        slum_id=slum_code[0][0]
    ).values_list("activity_type", "household_number")
    try:
        ...
        for (
            ...
            act_data
        ):  # here we are processing data where we have household numbers in json field.
        ...
        for (
            ...
            comm_avni
        ):  # here we are processing data where we have household numbers as str.
            if activity_record[0] in activity_data_with_hh:
                temp = activity_data_with_hh[activity_record[0]]
        ...
        hh_activity_cnt = (
            {}
        )  # counting number of distinct activity attended by the household.
    ...
    for record in dataList:
        if (
            ...
            key_lst = {
                ...
                "phase_two_material_date": "2",
                "phase_three_material_date": "3",
            }
            for k, v in key_lst.items():
                if k in record:
                    if (
                        "invoice_phase_" + v in record
                        and record["invoice_phase_" + v] == "Yes"
                    ):
            ...
            if (
                "factsheetAssignStatus" in record
                and record["factsheetAssignStatus"] != "No"
            ):
...
def ProcessShortView(request, slum_details=0):
    try:
        ...
        global slum_code
        slum_code = Slum.objects.filter(pk=int(request.GET["slumname"])).values_list(
            "id", "name", "electoral_ward__administrative_ward__city__name__city_name"
        )
        slum_funder = SponsorProjectDetails.objects.filter(
            slum=slum_code[0][0]
        ).exclude(sponsor__id=10)
        invoice_data = InvoiceItems.objects.filter(slum_id=slum_code[0][0]).values_list(
            "phase", "household_numbers"
        )

        householdData = HouseholdData.objects.filter(
        ...
        for (
            ...
            if hh_key in check_formdict:
                ...
                temp_dict = {
                    "household_number": k,
                    "occupancy_status": "RHS Not Done",
                    "total_moilization_acticity": v,
                }
        ...
        for i in daily_reporting_data:  # Prpcessing Daily Reporting data.
            ...
            if hh_key in check_formdict:
                ...
                if i["phase_two_material_date_str"]:  # Checking completion delayed.
                    if not i["completion_date_str"] and is_delayed(
                        i["phase_two_material_date_str"]
                    ):
                ...
                if (
                    i["p1_material_shifted_to"]
                    or i["p2_material_shifted_to"]
                    or i["p3_material_shifted_to"]
                ):  # Checking material is shifted or not.
                    temp["material_shifted"] = "Yes"
                else:
                    temp["material_shifted"] = "No"
                if i["status"] is not None:  # Checking Status of the household
                    if i["status"].strip() != "":
                        temp["toilet_status"] = ToiletConstruction.get_status_display(
                            i["status"]
                        )
...
def AnalyseGisTabData(slum_id):
    try:
        global slum_code
        slum_code = Slum.objects.filter(pk=int(slum_id)).values_list(
            "id", "name", "electoral_ward__administrative_ward__city__name__city_name"
        )
        slum_funder = SponsorProjectDetails.objects.filter(
            slum=slum_code[0][0]
        ).exclude(sponsor__id=10)
        Toilet_data = ToiletConstruction.objects.filter(
            slum=slum_code[0][0], phase_one_material_date__isnull=False
        ).exclude(agreement_cancelled=True)
        householdData = HouseholdData.objects.filter(
            slum_id=slum_code[0][0], rhs_data__isnull=False
        )
        columns_lst = [
            ...
            "house_structure",
            "ownership_status",
            "name_head_of_he_household",
            "isAadharCard",
            "aadhar_number",
            "color_of_ration_card",
            "girls_child",
            "Select Religion",
            "If other religion",
            "Select Caste",
            ...
            "HH can connect to Drainage line",
            "house_area",
            "Do you have individual water connection at home?",
            "Type_of_water_connection",
            "Facility_of_solid_waste_collection",
            "Do you have a toilet at home?",
            "Status of toilet under SBM ?",
            "Where the individual toilet is connected to ?",
            "Who all use toilets in the household ?",
            "Reason for not using toilet ?",
            "current_place_of_defication",
            "Is there availability of drainage to connect it to the toilet?",
            "Are you interested in an individual toilet ?",
            "Which CTB do your family members use ?",
            ...
            "sponsor_project",
            "factsheet_done",
            "family_factsheet_name",
            "toilet_connected_to",
            "Have you upgraded your toilet/bathroom/house while constructing individual toilet?",
            "Cost of upgradation",
            "activity_count",
            "HH can connect to Drainage line",
            "Floor Type",
        ...
        def getToiletData(record):
            ...
            if record.status is not None and record.status.strip() != "":
                data["final_status"] = ToiletConstruction.get_status_display(
                    record.status
                )
        ...
        for dct in formdict:
            if dct.get("occupancy_status") == "Occupied house":
                ...
                if hh in followup_data:
                    temp = followup_data[hh]
                    sanitation_data = {
                        sanitation_keys[key]: temp["followup_data"][key]
                        for key in sanitation_keys.keys()
                        if key in temp["followup_data"]
                    }
                ...
                diff_keys = set(columns_lst).difference(
                    set(dct.keys())
...
def gisDataDownload(request):
    ...
    if not city_id:
        ...
        columns_lst = [
            ...
            "total male members",
            "total other gender members",
            "Do you have any girl child/children under the age of 18?",
            "How many ? ( Count )",
            "Is there any seperated woman in the household ?",
            "Is there any widow woman in the household?",
            "Is any family member physically/mentally challenged?",
            ...
            "group_el9cl08/Facility_of_solid_waste_collection",
            "Plus code of the house",
            "Date of Survey for sanitation",
            "Type of household toilet ?",
            "Are you interested in an individual toilet?",
            "Current place of defecation",
            "Does any member of the household go for open defecation ?",
            "Do you have a toilet at home?",
            "If no for individual toilet , why?",
            "Under what scheme would you like your toilet to be built ?",
            "Does any household member have any of the construction skills give below?",
            "Have you applied for an individual toilet under SBM?",
            "How many installments have you received?",
            "When did you receive your first installment?",
            "When did you receive your second installment?",
            "When did you receive your third installment?",
            "If built by a contractor, how satisfied are you?",
            "Status of toilet under SBM?",
            "What was the cost incurred to build the toilet?",
            "Which CTB",
...
def addSponsor(request):
    ...
    def update_sponsor_project_details(
        sponsorProjectdetails_obj, household_code_updated
    ):
        old_household_code = sponsorProjectdetails_obj[0].household_code
        household_code_updated = merge_household_codes(
            old_household_code, household_code_updated
        )
        try:
            sp_details_obj = sponsorProjectdetails_obj.update(
                household_code=household_code_updated
            )
        except Exception as e:
            logger.error(e)
    ...
    def create_sponsor_project_details_subfields(aurgument_lst):
        ...
        create_sp_details_sub = SponsorProjectDetailsSubFields(
            sponsor_project_details_id=sponsor_project_details_id,
    ...
    sponsorProjectdetails = SponsorProjectDetails.objects.filter(
        sponsor_project_id=sponsor_project_id, slum_id=slum
    )
    if sponsorProjectdetails.exists():
        return_url_id = sponsorProjectdetails[0].id
        sponsorProjectdetailsSubfields = SponsorProjectDetailsSubFields.objects.filter(
            sponsor_project_details_id=sponsorProjectdetails[0].id,
            quarter=str(quarter_id),
        )
        if sponsorProjectdetailsSubfields.exists():
            ...
            try:
                sp_update = SponsorProjectDetailsSubFields.objects.filter(
                    id=sponsorProjectdetailsSubfields_obj.id
                ).update(
                    household_code=household_code_updated,
                    count=len(household_code_updated),
                )
                """ Now Update the household_code list in the SponsorProjectDetails"""
                update_sponsor_project_details(
                    sponsorProjectdetails, household_code_updated
                )
            except Exception as e:
                logger.error(e)
        else:
            """In This case there is no Sponsor Project Details SubFields entry available. So we have to create a new entry."""
            create_sponsor_project_details_subfields(
                [sponsorProjectdetails[0].id, household_code, quarter_id]
            )
            """ Now Update the household_code list in the SponsorProjectDetails"""
        ...
        create_sp_details = SponsorProjectDetails(
        ...
        create_sponsor_project_details_subfields(
            [create_sp_details.pk, household_code, quarter_id]
        )
        return_url_id = sp_details_pk
    admin_url = settings.BASE_APP_URL + reverse(
        "admin:sponsor_sponsorprojectdetails_change", args=[int(return_url_id)]
    )
