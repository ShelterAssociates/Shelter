    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["slumname"] = forms.ModelChoiceField(
            ...
            widget=ForeignKeyRawIdWidget(
                rel=ManyToOneRel(Slum._meta.get_field("id"), Slum, "id"),
...
class account_find_slum(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["account_slumname"].widget.attrs.update(
            {"name": "account_slumname", "id": "id_account_slumname"}
        )
        ...
        self.fields["account_cityname"].widget.attrs.update(
            {"name": "account_cityname", "id": "account_cityname"}
        )
...
class gis_tab(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        self.fields["gisdata_slumname"] = forms.ModelChoiceField(
            queryset=Slum.objects.all(),
            widget=ForeignKeyRawIdWidget(
                rel=ManyToOneRel(Slum._meta.get_field("id"), Slum, "id"),
                admin_site=admin.site,
        ...
        self.fields["gisdata_cityname"] = forms.ModelChoiceField(
            queryset=City.objects.all()
        )
        self.fields["gisdata_slumname"].widget.attrs.update(
            {"class": "customized-form"}
        )
        self.fields["gisdata_slumname"].widget.attrs.update(
            {"name": "gisdata_slumname", "id": "id_gisdata_slumname"}
        )
        self.fields["gisdata_slumname"].label = "Select slum"
        self.fields["gisdata_cityname"].widget.attrs.update(
            {"class": "customized-form"}
        )
        self.fields["gisdata_cityname"].widget.attrs.update(
            {"name": "gisdata_cityname", "id": "gisdata_cityname"}
        )
...
class SponsorForm(forms.Form):
    def __init__(self, *args, **kwargs):
        ...
        Sponsor_Name_List.extend(
            list(set(Sponsor.objects.all().values_list("id", "organization_name")))
        )
        self.fields["sponsor_name"] = forms.ChoiceField(
            choices=Sponsor_Name_List,
        ...
        self.fields["sponsor_name"].widget.attrs.update(
            {"name": "sponsor_name", "id": "sponsor_name"}
        )
        SponsorProject_Name_List = [
            ("0", "---select---"),
        ]
        self.fields["sponsor_project"] = forms.ChoiceField(
            choices=SponsorProject_Name_List,
            widget=forms.Select(attrs={"style": "width: 200px;"}),
        )
        self.fields["sponsor_project"].widget.attrs.update({"class": "customized-form"})
        self.fields["sponsor_project"].label = "Select Project"
        self.fields["sponsor_project"].widget.attrs.update(
            {"name": "sponsor_project", "id": "sponsor_project"}
        )
        quarter_choices = [
            ("0", "---select---"),
        ] + list(QUARTER_CHOICES)
        self.fields["sponsor_quarter"] = forms.ChoiceField(
        ...
        self.fields["sponsor_quarter"].widget.attrs.update(
            {"name": "sponsor_quarter", "id": "sponsor_quarter"}
        )
